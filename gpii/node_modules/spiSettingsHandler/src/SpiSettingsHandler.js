...
/*
 * Contains information about the most commonly used Windows data types.
 */
windows.types = {
    "BOOL": {
        size: 4,
...
windows.populateResults = function (payload, isNewValue, results) {
    ...
    if (type === "array") {
        ...
        var size = windows.types[payload.options.pvParam.valueType].size;
    }
    ...
    for (var property in payload.settings) {
        ...
        var valueToSet;
        if (type === "struct") {
            valueToSet = systemSettings[property];
        } else if (type === "array") {
            valueToSet = systemSettings[windows.types[payload.options.pvParam.valueType].read](offset);
        } else {
            valueToSet = systemSettings;
        }
        
        results[property][isNewValue ? "newValue" : "oldValue"] = valueToSet;
...
windows.getPvParam = function (payload) {    
    ...
    if (type === "struct") {
        ...
        for (var value in payload.settings) {
            pvParam[windows.types[payload.options.pvParam.valueType].write](payload.settings[value], index);
            index += size;
        }
        
    } else if (type in windows.types) {
        pvParam = new Buffer(windows.types[type].size);
        for (var fieldName in payload.settings);
        pvParam[windows.types[type].write](Number(payload.settings[fieldName]), 0);
...
windows.getCurrentSettings = function (payload) {
    ...
    if (type === "struct") {        
        ...
        user32.SystemParametersInfoW(getAction, uiParam, systemSettings, 0);
    }
