...
gpiiProcess.startChildProcesses = function () {
    for (var key in service.config.processes) {
        var proc = Object.assign({ key: key }, service.config.processes[key]);
        gpiiProcess.startChildProcess(proc);
    }
};
 ...
 * @param restarting {boolean} true if this process has terminated, and is being restarted.
 * @return {Promise} Resolves (with the pid) when the process has started.
 */
gpiiProcess.startChildProcess = function (procConfig, restarting) {
    var childProcess = restarting && gpiiProcess.childProcesses[procConfig.key];
...
gpiiProcess.stopChildProcesses = function () {
    ...
    console.log(Object.keys(gpiiProcess.childProcesses));
    for (var key in Object.keys(gpiiProcess.childProcesses)) {
        gpiiProcess.stopChildProcess(key);
    }
};
    ...
    gpiiProcess.monitorProcess(childProcess.pid).then(function () {
        ...
        if (!childProcess.shutdown) {
            ...
            if (seconds > 20) {
                ...
                if (childProcess.failureCount > 2) {
            ...
            if (restart) {
                // Delay restart it.
                setTimeout(gpiiProcess.startChildProcess, gpiiProcess.throttleRate(childProcess.failureCount),
                    childProcess.procConfig, true);
            }
        }
    });
};
 ...
 * @return {Number} Returns 10 seconds for every restart.
 */
...
/**
 * Stops a monitored process from being monitored. The promises for the process will resolve with "cancelled".
 * @param pid {Number} The process ID.
 */
gpiiProcess.unmonitorProcess = function (pid) {
    var resolves = [];
    for (var key in gpiiProcess.monitoredProcesses) {
        var proc = !isNaN(key) && gpiiProcess.monitoredProcesses[key];
        if (proc && proc.pid === pid) {
            resolves.push(proc.resolve);
            delete gpiiProcess.monitoredProcesses[key];
        }
    }
    ...
    if (handles.length <= 1) {
        ...
        windows.waitForMultipleObjects(handles).then(function (handle) {
            ...
            if (proc.isEvent) {
                ...
                delete gpiiProcess.monitoredProcesses[handle];
            ...
            if (last) {
                if (gpiiProcess.lastProcess.reject) {
                    delete gpiiProcess.monitoredProcesses[last.handle];
                ...
                Object.keys(gpiiProcess.monitoredProcesses).forEach(function (proc) {
                    if (!proc.isEvent) {
                        delete gpiiProcess.monitoredProcesses[proc.handle];
                        if (proc.reject) {
                            proc.reject(reason);
                        }
                    }
