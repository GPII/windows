...
windows.getSettingsInStructure = function (action, uiParam, structName) {
    ...
    //FIXME returns false when called with NONCLIENTMETRICS; cause - size of LOGFONT should be 92 instead of 32
...
windows.setFlagValues = function (payload) {
    ...
    var callSuccessful = user32.SystemParametersInfoW(
        action,
        uiParam,
        pvParam.type === "struct" ? pvParam.ref() : pvParam,
        0
    );
...
windows.populateResults = function (payload, isNewValue, results) {
    ...
    } else if (type === "BOOL" || type === "UINT" || type === "DWORD" || type === "ULONG") {
        var settingsStruct = new Buffer(4);
        user32.SystemParametersInfoW(getAction, uiParam, settingsStruct, 0);
    
    } else if (type === "array") {
        var settingsStruct = new Buffer(payload.options.pvParam.length * 4);
        user32.SystemParametersInfoW(getAction, uiParam, settingsStruct, 0);
    }
    ...
    for (var property in payload.settings) {
        ...
        var valueToSet = type === "struct" ? settingsStruct[property] : type === "array" ? settingsStruct.readInt32LE(index) : settingsStruct;
        if (isNewValue) {
...
windows.getPvParam = function (payload) {    
    ...
    if (type === "struct") {
        ...
        pvParam = new Buffer(4);
        //pvParam.writeInt32LE(Number(payload.settings.value));
        for (var fieldName in payload.settings) ;
        pvParam[writeValueType[type]](Number(payload.settings[fieldName]), 0);
        
    } else if (type === "array") {
        var size;
        if (payload.options.pvParam.valueType === "INT") {
            size = 4;
        }
        ...
        for (var value in payload.settings) {
            pvParam.writeInt32LE(payload.settings[value], index);
            index += size;
        }
