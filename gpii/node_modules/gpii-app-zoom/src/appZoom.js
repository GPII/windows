...
windows.appZoom.sendZoom = function (that, direction) {
    ...
    if (config) {
        ...
        if (config.ctrl) {
            var delay = config.wheel && config.wheel.delay || 0;
            windows.appZoom.pressControl(delay * 2);
        }
...
/**
 * Simulate pressing the control key down, and release it after a short delay.
 *
 * @param {Number} delay Milliseconds to wait before releasing it. [default: 10]
 */
windows.appZoom.pressControl = function (delay) {
...
windows.appZoom.findUncoveredPoint = function (hwnd, window) {
    ...
    if (rect) {
        ...
        if (windowOver !== hwnd && windowPid !== window.pid) {
            ...
            if (points.length === 1) {
                ...
                fluid.each(points, function (pt) {
                    ...
                    // var windowAtPoint = windows.user32.WindowFromPoint(pt.x, pt.y);
                    // var covered =  windowAtPoint !== hwnd && windows.getWindowProcessId(windowAtPoint) !== window.pid;
                    // if (covered) {
                    //     // That point is still covered by another window, penalise this point (maybe that window is ok?).
                    //     dist *= 4;
                    // }
