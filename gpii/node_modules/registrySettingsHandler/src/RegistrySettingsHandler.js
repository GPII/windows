 ...
 * @param {function (name, data, type)} callback A call-back for each value found, returning true to break the enumeration.
 * @return {*} An object containing the name, data, type, and size of each value.
 */
windows.enumRegistryValues = function (baseKey, path, callback) {
    var togo = {};
    var keyHolder = new Buffer(4);
    var openError = windows.openRegistryKey(baseKey, path, keyHolder);
    if (openError) {
    ...
    try {
        ...
        for (var index = 0; ; index++) {
            ...
            if (code2 === 2) { // FILE_NOT_FOUND
                return togo;
            } else if (code2 === 259) { // ERROR_NO_MORE_ITEMS
                break;
            } else {
                cr(code2);
            }
            ...
            if (callback && callback(value.name, value.data, value.type)) {
                break;
            }
        }
    } finally {
