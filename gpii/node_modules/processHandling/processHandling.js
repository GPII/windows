...
gpii.windows.findProcessByName = function (filename, all) {
    ...
    gpii.windows.eachProcess(function (processName, processId) {
        if (processName === filename) {
            matches.push(processId);
            // Break early if only 1 process is needed
            return !all;
        }
    });
...
/**
 * Enumerates all of the running processes.
 *
 * @param callback {function(string, int)} Function to call for every process passing the process name and ID, returning
 *   true to stop the enumeration early.
 */
gpii.windows.eachProcess = function (callback) {
    ...
    try {
        ...
        while (hRes) {
            ...
            var stop = callback(processName, pEntry.th32ProcessID);
            if (stop) {
                return;
            }
