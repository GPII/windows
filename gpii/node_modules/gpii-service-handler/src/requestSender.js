...
fluid.defaults("gpii.windows.service.requestSender", {
    ...
    invokers: {
        sendRequest: {
            ...
            args: [ "{serviceHandler}", "{arguments}.0", "{arguments}.1" ] // requestName, requestData
        },
        execute: {
 ...
 * @param {String} requestName Name of the request for the service.
 * @param {Object} requestData Request data.
 * @return {Promise} Promise resolving with the response.
 */
gpii.windows.service.sendRequest = function (service, requestName, requestData) {
    if (service.session) {
        var serviceRequest = {
            requestName: requestName,
            requestData: requestData
        };
        fluid.log("Service: sending request ", serviceRequest.requestName);
        return service.session.sendRequest(serviceRequest);
    } else {
