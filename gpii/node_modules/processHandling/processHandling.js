...
/**
<<<<<<< HEAD
 * Starts a process given a path an several options.
 *
 * @param path {String} The path to the executable to be launch.
 * @param options {Object} The options.
 * @param options.retries The number of retries, trying to launch the process.
 * @param options.pollDelay The number of milliseconds to wait between retries.
 * @return {promise} The promise will resolve when the process has been started,
 * if after all the retries it hasn't been initiated, it will be rejected.
 */
gpii.windows.startProcessByPath = function (path, options) {
    var defaultOptions = {
        retries: 1,
        pollDelay: 100
    };
    ...
    var retry = function (retries) {
        while (retries > 0) {
            ...
            if (proc.pid) {
                ...
                setTimeout(retry, options.pollDelay, retries - 1);
            }
        }
        promise.reject({
            isError: true,
            message: "Unable to startProcess '" + path + "'."
        });
    }
...
/**
 * Restarts a process given its name.
 *
 * @param name {String} The name of the process to be restarted.
 * @param options {Object} The options.
 * @param options.retries The number of retries for trying to restart the process after
 *  being closed.
 * @param options.pollDelay The number of milliseconds to wait between restart retries.
 * @param options.timeout The number of milliseconds to wait for the process to close.
 * @return {promise} The promise will resolve when the process have been started again after
 *  being closed, if the timeout expires, or all the retries are used, it will reject.
 */
gpii.windows.restartProcessByName = function (name, options) {
    var promise = fluid.promise();

    var procPid = gpii.windows.findProcessByName(name, false, false);
    if (procPid === null) {
        promise.reject({
            isError: true,
            message: "restartProcessByName: Not able to find process pid."
        });
        return promise;
    }

    var path = gpii.windows.getProcessPath(procPid);
    if (path === null) {
        promise.reject({
            isError: true,
            message: "restartProcessByName: Not able to retrieve process path."
        });
        return promise;
    }

    gpii.windows.closeProcessByName(name, options).then(
        function () {
            gpii.windows.startProcessByPath(
                path,
                {
                ...
                function (err) {
                    promise.reject({
                        isError: true,
                        message: "restartProcessByName: " + err
                    });
                }
            );
        },
 ...
 * @param options.pollDelay How long to wait between processes check.
...
>>>>>>> 02de41cc53f7e6d22dd660c4937b8812bb1be458
