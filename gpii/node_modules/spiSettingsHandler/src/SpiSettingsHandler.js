...
// Most calls to SystemParametersInfoW require pvParam to be a pointer,
// however some, such as SPI_SETMOUSECLICKLOCK, need a value to be passed
// directly in pvParam.
var systemParametersInfoWPtr = ffi.Library("user32", {
    "SystemParametersInfoW": [
        "int32", [ "uint32", "uint32", "void*", "uint32" ]
    ]
});
var systemParametersInfoWInt = ffi.Library("user32", {
    "SystemParametersInfoW": [
        "int32", [ "uint32", "uint32", "int32", "uint32" ]
    ]
});
 ...
 */
spi.getCurrentSettings = function (payload) {
    var getAction = windows.actionConstants[payload.options.getAction];
    var uiParam = spi.getUiParam(payload);
    var pvParam = spi.createPvParam(payload);

    var callSuccessful = systemParametersInfoWPtr.SystemParametersInfoW(getAction, uiParam, pvParam, 0);
    ...
    currentSettings.pvParam = (payload.options.pvParam.type === "array") ?
        windows.bufferToArray(pvParam, payload.options.pvParam.valueType) :
        pvParam.deref();
 ...
 * set action.
 */
spi.applySettings = function (payload) {
    var action = windows.actionConstants[payload.options.setAction];
    var uiParam = spi.getUiParam(payload);
    var pvParam = spi.getPvParam(payload);

    uiParam = pvParam.uiParam; // this will be updated because it looks bad
    pvParam = pvParam.pvParam;

    var callSuccessful = false;
    if (action === windows.actionConstants.SPI_SETMOUSECLICKLOCK ||
            action === windows.actionConstants.SPI_SETMOUSETRAILS) {
        callSuccessful = systemParametersInfoWInt.SystemParametersInfoW(action, uiParam, pvParam, 0);
    } else {
        callSuccessful = systemParametersInfoWPtr.SystemParametersInfoW(action, uiParam, pvParam, 0);
    }
 ...
 */
spi.populateResults = function (payload, isNewValue, isGetting, results) {
    var systemSettings = spi.getCurrentSettings(payload);
    ...
    for (var currentSetting in payload.settings) {
        ...
        var valueToSet = windows.resolvePath(systemSettings, path);
 ...
 * payload.
 */
spi.getUiParam = function (payload) {
    ...
    if (uiParam === "struct_size") {
        var result = windows.structures[payload.options.pvParam.name].size;
        if (payload.options.pvParam.name === "NONCLIENTMETRICS" && os.release() < "6") {
            result -= 4; // do not unclude NONCLIENTMETRICS.iPaddedBorderWidth
        }
        return result;
    }
 ...
 */
spi.createPvParam = function (payload) {
    var pvParam;

    switch (payload.options.pvParam.type) {
    case "struct":
        pvParam = windows.createEmptyStructure(payload.options.pvParam.name).ref();
        break;

    case "array":
        var length = payload.options.pvParam.length;
        var size = ref.types[windows.types[payload.options.pvParam.valueType]].size;
    ...
    default:
        if (payload.options.pvParam.type in windows.types) {
            pvParam = ref.alloc(windows.types[payload.options.pvParam.type]);
        } else {
 ...
 */
spi.getPvParam = function (payload) {
    var systemSettings = spi.getCurrentSettings(payload);
    ...
    for (var currentSetting in payload.settings) {
        ...
        windows.resolvePath(systemSettings, path, payload.settings[currentSetting].value);
    }

    if (payload.options.pvParam.type === "array") {
        pvParam = windows.arrayToBuffer(pvParam, payload.options.pvParam.valueType);
    } else if (payload.options.pvParam.type in windows.types) {
 ...
 */
spi.setImpl = function (payload) {
    windows.defineStruct(payload);

    var results = {};
    spi.populateResults(payload, false, false, results);

    spi.applySettings(payload);

    spi.populateResults(payload, true, false, results);

    // transform results here   oldValue: x   -->   oldValue: { value: x, path: ... }
    fluid.each(results, function(value, setting) {
        // this is a perfect candidate for fluid.transform()
...
gpii.windows.spiSettingsHandler.set = function (payload) {
    return gpii.windows.invokeSettingsHandler(spi.setImpl, payload);
};

spi.getImpl = function (payload) {
    windows.defineStruct(payload);
    var results = {};
    spi.populateResults(payload, false, true, results);

    return results;
};

gpii.windows.spiSettingsHandler.get = function (payload) {
    return gpii.windows.invokeSettingsHandler(spi.getImpl, payload);
};
...
gpii.windows.spiSettingsHandler.updateCursors = function () {
    ...
    systemParametersInfoWInt.SystemParametersInfoW(0x57, 0, 0, 0);
};
...
if (process.argv[1].indexOf("piSettingsHandler") !== -1 && process.argv.length === 3) {
    ...
    var returnCode = spi.setImpl(jsonFile);
