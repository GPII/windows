...
gpii.windows.iod.stopApplication = function (that, service) {
    ...
    if (that.packageInfo.stop) {
        var cmd, args;
        if (typeof(that.packageInfo.stop) === "string") {
            cmd = that.packageInfo.stop;
            args = [];
        } else {
            cmd = that.packageInfo.stop.cmd;
            args = fluid.makeArray(that.packageInfo.stop.args);
        }
