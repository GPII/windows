...
gpii.windows.spi.checkWallpaperPayload = function (payload) {
    if (!fluid.get(payload, "settings.Wallpaper.value")) {
        return false;
    }
...
gpii.windows.spi.applySettings = function (payload) {
    ...
    if (action === gpii.windows.actionConstants.SPI_SETDESKWALLPAPER) {
        ...
        if (!validPayload) {
            fluid.fail("gpii.windows.spi.applySettings: Invalid PvParam payload '" + pvParam + "'");
        }
    }
        ...
        .then(function () {
            ...
            if (sendChange || (problematicSpiCalls.indexOf(action) >= 0)) {
                ...
                if (callSuccessful) {
                    ...
                    var errorCode = gpii.windows.kernel32.GetLastError();
                    fluid.log("SpiSettingsHandler.js: spi.applySettings() failed with error code " + errorCode + ".");
                }
            }
        });
