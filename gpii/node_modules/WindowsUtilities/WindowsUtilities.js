...
windows.resolvePath = function (object, path, setValue) {
    ...
    for (var i = 0; i < fields.length - 1; ++i) {
        if (fields[i] === "dwFlags") {
            return windows.accessFlag(object, fields[i + 1], setValue);
        }
        object = windows.accessField(object, fields[i]);
    }
    
    return windows.accessField(object, fields[fields.length - 1], setValue);
};

/**
 * Accesses a single field of an object.
 * 
 * @param {Object} object The object to be accessed.
 * @param {String} field A string containing the field name.
 * @param {Object} setValue <optional> A new value to be stored in the field.
 * @return {Object} The value stored in the field.
 */
windows.accessField = function (object, field, setValue) {
    ...
    if (setValue !== undefined) {
        if (typeof setValue === "string") {
            object[field] = ref.allocCString(setValue, "ucs2");
        } else {
            object[field] = setValue;
        }
    }
    
    return Buffer.isBuffer(object[field]) ? object[field].toString("ucs2") : object[field];
};
...
windows.arrayToBuffer = function (array, type) {
    ...
    for (var i = 0; i < array.length; ++i) {
        ref.set(buf, i * size, array[i], type);
    }
...
windows.bufferToArray = function (buffer, type) {
    ...
    for (var i = 0; i < buffer.length / size; ++i) {
        array[i] = ref.get(buffer, i * size, type);
    }
