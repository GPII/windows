...
jqUnit.test("Testing getUsbDrives", function () {
    ...
    // Make getUsbDrives think a drive is mounted, according to the current test.
    var checkMountedDrive = gpii.windows.checkMountedDrive;
    gpii.windows.checkMountedDrive = function (path) {
        var test = tests[path];
        return test ? !test.unmounted : checkMountedDrive(path);
    };
    ...
    try {
        ...
        gpii.windows.checkMountedDrive = checkMountedDrive;
...
jqUnit.test("Test icon spacing", function () {
    require("../../spiSettingsHandler/index.js");

    var origValue = gpii.windows.getIconSpacing();
    jqUnit.assertTrue("origValue.x must be a number", !!parseInt(origValue.x));
    jqUnit.assertTrue("origValue.y must be a number", !!parseInt(origValue.y));

    // Set it to a new value
    var iconSpacing = { x: 78, y: 80 };
    gpii.windows.setIconSpacing(iconSpacing);
    var newValue = gpii.windows.getIconSpacing();
    jqUnit.assertDeepEq("newValue must the new value", iconSpacing, newValue);

    // Set it back to the original value
    gpii.windows.setIconSpacing(origValue, true);
    var revertedValue = gpii.windows.getIconSpacing();
    jqUnit.assertTrue("revertedValue must be the original value", origValue, revertedValue);
});
