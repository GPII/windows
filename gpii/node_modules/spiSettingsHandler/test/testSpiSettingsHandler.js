...
var testPayloads = ["testMouseTrails", "testFilterKeys", "testMouse", "testMouseClickLock", "testLogFont",
    "testNonClientMetrics", "testStickyKeys", "testHighContrast", "testHighContrastToEmpty", "testMouseKeys", "testWallpaper"];

jqUnit.asyncTest("SPISettingsHandler test - Wallpaper path checking", function () {
    var value = fluid.module.resolvePath("%gpii.windows.spiSettingsHandlerModule/../test");
    var res = gpii.windows.spi.checkWallpaperPath(value);
    jqUnit.assertDeepEq("Assert testWallpaperPathCheck:'" + value + "' is invalid", res, false);

    value = fluid.module.resolvePath("%gpii.windows.spiSettingsHandlerModule/../test/testWallpaper.jpg");
    res = gpii.windows.spi.checkWallpaperPath(value);
    jqUnit.assertDeepEq("Assert testWallpaperPathCheck:'" + value + "' is valid", res, true);
});
...
fluid.each(testPayloads, function (name) {
    ...
    jqUnit.asyncTest("SpiSettingsHandler test - " + name, function () {
        ...
        if (payload.settings.wallpaper !== undefined) {
            payload.settings.wallpaper.value = fluid.module.resolvePath(payload.settings.wallpaper.value);
        }
