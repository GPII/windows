...
windows.isSupportedMsg = function(message) {
    ...
    return hwnd !== undefined && msg !== undefined;
}

windows.sendWindowMessage = function(message) {
    try {
        if (!windows.isSupportedMsg(message)) {
            fluid.log("resgistrySettingHandler: Received unsupported message '" + message + "'.");
            return;
        }
...
windows.registrySettingsHandler.setImpl = function (payload) {
    ...
    if (payload.options.message !== undefined) {
        windows.sendWindowMessage(payload.options.message);
    }
