...
// Native functions for colors

/**
 * Set the current system color for dekstop to the color passed as parameter.
 *
 * @param {Integer} color The color expressed as a single unsigned integer value.
 * @return {Integer} Returns non-zero in case of success, zero otherwise.
 */
handler.setSolidColor = function (color) {
    var uint32Color = color >>> 0;

    return handler.backgroundColorManager(["set", uint32Color], true);
};

/**
 * Get the current system color for dekstop.
 *
 * @return {Integer} The color expressed as a single unsigned integer value.
 */
handler.getSolidColor = function () {
    return handler.backgroundColorManager(["get"], true);
};
...
handler.setImpl = function (payload) {
    ...
    if (sysRequest[1] !== undefined) {
        ...
        var curSysColor = handler.getSolidColor();
                    ...
                    if (isSolidColor(payload)) {
                        getWallPr.then(
                            function (wallRes) {
                                ...
                                var sColorRes = handler.setSolidColor(newColor);
                                ...
                                function (spiResults) {
                                    getWallPr.then(
                                        function (wallRes) {
                                            ...
                                            fluid.set(handlerValues, "oldValues.image", oldWallpaper);
                                            fluid.set(handlerValues, "newValues.image", spiResults);
