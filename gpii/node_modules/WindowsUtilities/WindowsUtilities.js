 ...
 * "firefox.exe".
 */
windows.killProcessByName = function (filename) {
...
// TODO - split the file here - need to see how it is invoked.
(function () {
    /*"use strict";
    ...
    */
    ...
    var windowsScreenResolution = fluid.registerNamespace("gpii.windowsScreenResolution");
     ...
     */
    windowsScreenResolution.getScreenResolution = function () {
        var dm = new DEVMODEW();
        dm.ref().fill(0);
        dm.dmSize = DEVMODEW.size;

        if (c.FALSE != user32.EnumDisplaySettingsW(ref.NULL, c.ENUM_CURRENT_SETTINGS, dm.ref()))
        {
    ...
    /**
     *  Sets the current screen resolution if possible
     *
     * @param {Object} The new sreen resolution width and height
     * @return {boolean} true if successfu
     */
    windowsScreenResolution.setScreenResolution = function (newRes) {
        if (undefined === newRes ||
            null === newRes ||
        ...
        while (c.FALSE != user32.EnumDisplaySettingsW(ref.NULL, index++, dm.ref())) {
            if (dm.dmPelsWidth == newRes.width && dm.dmPelsHeight == newRes.height) {
                var dmCurrent = new DEVMODEW();
                dmCurrent.ref().fill(0);
                dmCurrent.dmSize = DEVMODEW.size;

                if (c.FALSE != user32.EnumDisplaySettingsW(ref.NULL, c.ENUM_CURRENT_SETTINGS, dmCurrent.ref())) {
                    ...
                    var lRet = user32.ChangeDisplaySettingsW(dmCurrent.ref(), 0);
    ...
    fluid.defaults("gpii.windowsScreenResolution.getScreenResolution", {
        gradeNames: "fluid.function",
        argumentMap: {
            filename: 0
        }
    ...
    fluid.defaults("gpii.windowsScreenResolution.setScreenResolution", {
        gradeNames: "fluid.function",
        argumentMap: {
            filename: 0
        }
    ...
    windowsScreenResolution.allSettings = {
        "screen-resolution": {
            get: "windowsScreenResolution.getScreenResolution",
            set: "windowsScreenResolution.setScreenResolution"
        }
    };

    var getImpl = function (settingsRequest) {
        settingsRequest = settingsRequest || windowsScreenResolution.allSettings;
        var settings = fluid.transform(settingsRequest, function (value, key) {
            var funcEntry = windowsScreenResolution.allSettings[key];
            if (funcEntry) {
                return fluid.invokeGlobalFunction(funcEntry.get);
            } else {
                fluid.fail("Invalid key to Windows screen resolution settings handler - " +
                    key + " - valid choices are " + JSON.stringify(fluid.keys(windowsScreenResolution.allSettings)));
            }
        });
        return settings;
    };

    windowsScreenResolution.get = function (settingsarray) {
        var app = fluid.copy(settingsarray);
        for (var appId in app) {
            for (var j = 0; j < app[appId].length; j++) {
                var settings = getImpl(app[appId][j].settings);
    ...
    windowsScreenResolution.set = function (settingsarray) {
        var app = fluid.copy(settingsarray);
        for (var appId in app) {
            for (var j = 0; j < app[appId].length; j++) {
                ...
                for (var settingKey in settings) {
                    ...
                    if (settingKey === "screen-resolution") {
                        oldValue = windows.getScreenResolution();
                        windows.setScreenResolution(value);
                    } else {
