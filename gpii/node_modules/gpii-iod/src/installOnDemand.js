...
// Mix-in grade for gpii.iod, providing Windows-related functionality.
fluid.defaults("gpii.iod.windows", {
    invokers: {
        getMountedVolumes: {
            funcName: "gpii.windows.getAllDrives"
        }
    }
});

// Base package installer for Windows.
fluid.defaults("gpii.windows.iod.packageInstaller", {
    ...
    invokers: {
        ...
        startElevatedProcess: {
            ...
            args: ["{serviceHandler}", "{arguments}.0", "{arguments}.1", "{arguments}.2" ]
        }
    }
});
 ...
 * @param {Boolean} options.desktop true to run on the desktop.
 * @return {Promise} Resolves when the process terminates.
 */
gpii.windows.iod.startElevatedProcess = function (service, command, args, options) {
    ...
    // Wait for service connectivity - uninstalls can occur quite early in the lifetime.
