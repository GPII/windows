...
gpii.windows.spi.setImpl = function (payload) {
    ...
    if (payload.settings.HighContrastOn) {
        ...
        if (doNothing) {
            ...
            var cachedFilesPath = path.join(env.APPDATA, "Microsoft\\Windows\\Themes\\CachedFiles");
            var optionsCached = { "recursive": true,  "silent": true, "local": true };

            var tWallpaperPath = path.join(env.APPDATA, "Microsoft\\Windows\\Themes\\TranscodedWallpaper");
            var optionsCached = { "silent": true, "local": true };

            gpii.windows.rm(cachedFilesPath, optionsCached);
            gpii.windows.rm(tWallpaperPath, optionsCached);
        }
    }
