...
gpii.windows.closeAfterExitCommand = function (command, args, processToWait, processToClose, options) {
    ...
    closeCommand.on('exit', function (code, signal) {
        // Linter (Bug) Workaround: Linter fails with arrow style lambda without parameters.
        // Linter (Bug): Expected fail - "error  Parsing error: Unexpected token >".
        gpii.windows.waitForProcessTermination(processToWait, {pollDelay: 100}).then(function() {
...
gpii.windows.closeAfterOpenCommand = function (command, args, processToWait, processToClose, options) {
    ...
    gpii.windows.waitForProcessStart(processToWait, {pollDelay: 100}).then(() => {
        gpii.windows.closeProcessByName(processToClose, {cleanOnly: false, timeout: 150})
            .then(() => {
