 ...
 *
 * typedef struct DISPLAYCONFIG_SET_DPI {
 *   DISPLAYCONFIG_DEVICE_INFO_HEADER header; // https://msdn.microsoft.com/en-us/library/windows/hardware/ff553920.aspx
 *   UINT32 dpiIndex; // 0 = 100% (96 DPI), 1 = 125% (120 DPI), 2 = 150%, etc.
 * } DISPLAYCONFIG_SET_DPI;
 *
 * The 'type' member of the header, which is used to identify the type of packet, is (uint) -4.
 *
 * The procedure to getting the current DPI is similar. The usual way of getting it is using
 ...
 *
 * typedef struct DISPLAYCONFIG_GET_DPI {
 *   DISPLAYCONFIG_DEVICE_INFO_HEADER header; // https://msdn.microsoft.com/en-us/library/windows/hardware/ff553920.aspx
 *   UINT32 unknown;     // Comes back as zero - the meaning is unknown.
 *   UINT32 dpiIndex;    // 0 = 100% (96 DPI), 1 = 125% (120 DPI), 2 = 150%, etc.
 ...
 * @return {{configured, maximum, actual}} The newly configured, actual, and maximum DPI scale.
 */
 ...
 *
 * The maximum scale is the highest DPI scale that the current screen resolution supports. The actual scale is the
 * configured scale, capped at the maximum (until the resolution increases).
 *
