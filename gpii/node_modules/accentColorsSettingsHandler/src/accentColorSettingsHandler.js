 ...
 *
 * This settings handler handles the system accent color and the following
 * related settings:
 *   - Windows transparency. Accessed through the systemSettingsHandler.
 *   - Color prevalence. Accessed through the systemSettingsHandler.
 *   - Color prevalence for title bars. Accessed through the systemSettingsHandler.
 *   - Windows 10 Apps color theme. Light or dark.
 *
 * This settings are included into this settings handler, because they modify how
 * the system displays the current accent color, or where it does it.
 *
 * In the implementation of this setting handler, was necessary the usage of C# to
 * ensure compatibility with the calling binaries with the use of the compiler
 * pragma: "CallingConvention = CallingConvention.StdCall". Also, was necessary to
 * access to functions exported by "ordinal" instead of "symbol" in certain binaries, performing
 * a link with this kind of binary exporting technique is simpler.
 *
