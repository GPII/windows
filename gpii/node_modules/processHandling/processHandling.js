...
gpii.windows.closeProcessWithMinLevel = function (promise, processToClose) {
    var procPids = gpii.windows.findProcessByName(processToClose, true);

    var tryCloseWithHighForce = function () {
        gpii.windows.closeApplications("high", {pids: procPids, restart: true})
            .then(
                function () {
                    promise.resolve();
                },
                function (err) {
                    promise.reject({
                        isError: true,
                        message: "Failed to close process with maximum level of force.",
                        error: err
                    });
                }
            );
    ...
    var tryCloseWithMediumForce = function () {
        gpii.windows.closeApplications("medium", {pids: procPids, restart: true})
            .then(
                function () {
                    promise.resolve();
                },
    ...
    gpii.windows.closeApplications("low", {pids: procPids, restart: true})
        .then(
            function () {
                promise.resolve();
            },
...
gpii.windows.retryCommandExec = function (promise, command, args, processToWait, processToClose, options, retries) {
    fluid.log("gpii.windows.closeAfterCommand: Open - Retry number " + retries + ".");
    ...
    if (retries === 0 || retries <= 0) {
        ...
        if (options.commandType === "Open") {
            gpii.windows.waitForProcessStart(processToWait, {timeout: 15000, pollDelay: 100})
                .then(
                    function () {
                        gpii.windows.closeProcessWithMinLevel(promise, processToClose);
                    },
                    function () {
                        gpii.windows.retryCommandExec(promise, command, args, processToWait, processToClose, options, retries - 1);
                    }
                );
        } else if (options.commandType === "Close") {
            gpii.windows.waitForProcessTermination(processToWait, {timeout: 15000, pollDelay: 100})
                .then(
                    function () {
                        gpii.windows.closeProcessWithMinLevel(promise, processToClose);
                    },
...
gpii.windows.closeAfterCommand = function (command, args, processToWait, processToClose, options) {
    ...
    // Check the command type that is going to be executed.
    ...
    gpii.windows.retryCommandExec(promise, command, args, processToWait, processToClose, options, options.retries);
