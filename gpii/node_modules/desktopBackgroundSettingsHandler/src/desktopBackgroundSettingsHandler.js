...
windows.desktopBackground.mapBackgroundType = function (sysResults, mode) {
    ...
    if (mode === "get") {
        ...
        var oldBackgroundTypeVal =
            sysResults[windows.desktopBackground.revIdMaps.BackgroundType].oldValue.value;
        var newBackgroundTypeVal =
            sysResults[windows.desktopBackground.revIdMaps.BackgroundType].newValue.value;
...
windows.desktopBackground.processSolidColorPayload = function (pRes, pWall, payload, handlerValues, sysResults, curMode) {
    pWall.then(
        function (wallRes) {
...
windows.desktopBackground.processPicturePayload = function (pRes, pWall, payload, handlerValues, sysResults, curMode) {
...
windows.desktopBackground.setImpl = function (payload) {
    ...
    if (sysRequest[1] !== undefined) {
        ...
        var pWallpaper = windows.desktopBackground.getDeskWallpaper();
        ...
        pSysResults.then(function (rawSysResults) {
            ...
            if (sysResults[1] !== undefined) {
                ...
                if (windows.desktopBackground.isSolidColor(payload)) {
                    windows.desktopBackground.processSolidColorPayload(pRes, pWallpaper, payload, handlerValues, sysResults, curMode);
                } else if (windows.desktopBackground.isPicture(payload)) {
                    windows.desktopBackground.processPicturePayload(pRes, pWallpaper, payload, handlerValues, sysResults, curMode);
                } else {
...
/**
 * Get current values for a solid color payload request.
 *
 * @param {*} pRes Promise to resolve when values are get from the system.
 * @param {*} sysResults The results from the systemSettingsHandler from which map the current values.
 * @param {*} curMode The current operation mode. Which is either "set" or "get".
 */
windows.desktopBackground.processGetSolidColorPayload = function (pRes, sysResults, curMode) {
    var curSysColor = windows.getSolidColor();
    var colorValues = { values: { color: curSysColor } };
    var colorResults = windows.desktopBackground.addInputVals(colorValues, sysResults, curMode);

    pRes.resolve(colorResults);
};

/**
 * Get current values for a picture payload request.
 *
 * @param {*} pRes Promise to resolve when values are get from the system.
 * @param {*} sysResults The results from the systemSettingsHandler from which map the current values.
 * @param {*} curMode The current operation mode. Which is either "set" or "get".
 */
windows.desktopBackground.processGetPicturePayload = function (pRes, sysResults, curMode) {
    var curWallpaperP = windows.desktopBackground.getDeskWallpaper();
    ...
    curWallpaperP.then(
        function (wallpaperRes) {
            ...
            var picResults = windows.desktopBackground.addInputVals(picValues, sysResults, curMode);

            pRes.resolve(picResults);
        },
        function (err) {
            pRes.reject("desktopBackgroundSettingsHandler: getImpl failed beacuse windows.desktopBackground.getDeskWallpaper failed with error - '" + err + "'");
        }
    );
};
        ...
        pSysResults.then(
            function (rawSysResults) {
                ...
                if (sysResults[1] !== undefined) {
                    ...
                    if (windows.desktopBackground.isSolidColor(payload)) {
                        windows.desktopBackground.processGetSolidColorPayload(pRes, sysResults, curMode);
                    } else if (windows.desktopBackground.isPicture(payload)) {
                        windows.desktopBackground.processGetPicturePayload(pRes, sysResults, curMode);
                    } else {
                        pRes.resolve(sysResults);
                    }
                }
