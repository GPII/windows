...
jqUnit.asyncTest("SpiSettingsHandler HighContrast Completion", function () {
    ...
    // Create the process, with a 10 second timeout
    shelljs.exec(waitExePath + " GpiiHighContrastTest /T 10 > nul", {
        async: true
    });

    // Begin the waiting
    gpii.windows.spi.waitForSpi(gpii.windows.actionConstants.SPI_SETHIGHCONTRAST, waitExe)
        .then(function (code) {
            jqUnit.assertEquals("Child process should return 0 (success)", 0, code);
            jqUnit.start(1);
        });


    // Stop the process
    shelljs.exec("waitfor /SI GpiiHighContrastTest");
