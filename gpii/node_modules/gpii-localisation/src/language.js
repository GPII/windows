 ...
 * @param {Array<String>} fullCodes A list of language codes all of which contain a region, to be used as a lookup to
 * complete a langCode which does not specify the region.
 * @return {String} The language code, in the correct casing.
 */
gpii.windows.language.fixLangCode = function (langCode, fullCodes) {
    var parts = (langCode && langCode.split) ? langCode.split("-") : [];

    // Handle the case where just a language is presented.
    if (parts.length === 1 && parts[0].length === 2) {
        var codePrefix = langCode.toLowerCase() + "-";
        var fullCode = fullCodes.find(function (fullCode) {
            return fullCode.toLowerCase().startsWith(codePrefix);
        });
        if (fullCode) {
...
gpii.windows.language.getInstalled = function (that) {
    ...
    // The language codes in the list of installed languages doesn't always contain the region part, for languages
    // which are used in a single region, such as Polish (pl) or Korean (ko). Fortunately, the list of recently used
    // languages does. The recently used language list can therefore be used as the lookup for region-less languages.

    // Get the languages that are installed (may contain region-less language codes).
    var installed = gpii.windows.enumRegistryKeys(
        "HKEY_CURRENT_USER", "Control Panel\\International\\User Profile");

    // Get the list of recently used languages (containing full language codes, but also removed languages).
    var fullCodes = gpii.windows.enumRegistryKeys(
        "HKEY_LOCAL_MACHINE", "SYSTEM\\CurrentControlSet\\Control\\MUI\\UILanguages");

    var langCodes = installed.map(function (lang) {
        return gpii.windows.language.fixLangCode(lang, fullCodes);
    });
