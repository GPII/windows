...
jqUnit.test("Testing Send Zoom on Min Window", function () {
    ...
    var oldIsIconic = gpii.windows.user32.IsIconic;
    try {
        gpii.windows.user32.IsIconic = function () {
            return true;
        };
        ...
        gpii.windows.user32.IsIconic = oldIsIconic;
    }
});
...
jqUnit.test("Testing Child Window Not Found", function () {
    ...
    var oldFluidLog = fluid.log;
    var logMessage;
    try {
        fluid.log = function (msg) {
            logMessage = msg;
        };
        appZoom.sendZoom("decrease");
        jqUnit.assertEquals("logMessage should show child window not found", "appZoom: child window 'InvalidChildWindow' not found.", logMessage);
    } catch (e) {
        jqUnit.fail("Error zooming childWindow with the following exception: " + e);
    } finally {
        fluid.log = oldFluidLog;
    }
});

jqUnit.test("Testing Child Window Found", function () {
    var appZoom = gpii.windows.appZoom({});
    appZoom.currentWindow = {
        hwnd: appZoom.getMessageWindow(),
        config: {
            childWindow: "SimulatedChildWindow"
        }
    };
    var oldGetClassNameW = gpii.windows.user32.GetClassNameW;
    var oldStringFromWideChar = gpii.windows.stringFromWideChar;
    var oldEnumerateWindows = gpii.windows.enumerateWindows;
    try {
        gpii.windows.user32.GetClassNameW = function () {
            return true;
        };
        gpii.windows.stringFromWideChar = function () {
            return "SimulatedChildWindow";
        };
        gpii.windows.enumerateWindows = function (whnd, cb) {
            return cb("SimulatedChildWindow");
        };
        appZoom.sendZoom("decrease");
        jqUnit.assertEquals("SimulatedChildWindow found", "SimulatedChildWindow", "SimulatedChildWindow");
    } catch (e) {
        jqUnit.fail("Error zooming childWindow with the following exception: " + e);
    } finally {
        gpii.windows.user32.GetClassNameW = oldGetClassNameW;
        gpii.windows.stringFromWideChar = oldStringFromWideChar;
        gpii.windows.enumerateWindows = oldEnumerateWindows;
    }
});
...
jqUnit.test("Testing Active Window", function () {
    ...
    var oldGetWindowInfo = gpii.windows.appZoom.getWindowInfo;
    var activeWindow;
    try {
        gpii.windows.appZoom.getWindowInfo = function () {
            return {
                pid: -1,
        ...
        gpii.windows.appZoom.getWindowInfo = oldGetWindowInfo;
    }
});
...
jqUnit.test("Testing Send Wheel", function () {
    ...
    var oldSetCursorPos = gpii.windows.user32.SetCursorPos;
    var currentPosition;
    try {
        gpii.windows.user32.SetCursorPos = function (x, y) {
        ...
        gpii.windows.user32.SetCursorPos = oldSetCursorPos;
    }
});
...
jqUnit.test("Testing Get Window Info", function () {
    var oldGetWindowProcessId = gpii.windows.getWindowProcessId;
    try {
        gpii.windows.getWindowProcessId = function () {
            return 1;
        };
        ...
        gpii.windows.getWindowProcessId = oldGetWindowProcessId;
    }
});

jqUnit.test("Testing Set Control Key State (down)", function () {
    var oldKeyBdEvent = gpii.windows.user32.keybd_event;
    var keyState;
    try {
        gpii.windows.user32.keybd_event = function () {
        ...
        gpii.windows.user32.keybd_event = oldKeyBdEvent;
    }
});
