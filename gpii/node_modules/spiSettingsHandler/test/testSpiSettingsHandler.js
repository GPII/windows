...
fluid.each(testPayloads, function (name) {
    jqUnit.test("SpiSettingsHandler test - " + name, function () {
        ...
        var oldSettings = spi.getImpl(payload);
        var newSettings = spi.setImpl(payload);
        ...
        jqUnit.assertDeepEq(name + " restored", oldSettings, spi.getImpl(payload));
    });
});
...
jqUnit.test("SpiSettingsHandler API test - ", function () {
    ...
    var result2 = windows.spiSettingsHandler.set(testData.set.payload);
    jqUnit.assertDeepEq("Checking that SET returns the correct payload", result2, testData.set.expectedResult);
