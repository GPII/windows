 ...
 */
windows.checkNewApplication = function (that, pid, exePath) {
 ...
 *
 * @param that {Component} The gpii.windowsMetrics instance.
 * @param currentProcess {Object} [Optional] The pid and exe file owning the currently active window.
 */
windows.checkActiveWindow = function (that, currentProcess) {
    var activePid = 0;
    var lastHwnd = 0;
    if (!currentProcess) {
        currentProcess = {};
    }
    var timeActivated = process.hrtime();
    ...
    windows.waitForCondition(function () {
        ...
        return activePid !== currentProcess.pid || !that.model.state.application.active;
    }, {
        pollDelay: that.model.config.application.precision
    }).then(function () {
        if (currentProcess) {
            var duration = process.hrtime(timeActivated);
            var data = {
        ...
        if (that.model.state.application.active) {
            ...
            // been launched.
            windows.checkNewApplication(that, activePid, exePath);
            windows.checkActiveWindow(that, {pid: activePid, exe: exePath});
        }
    });
};
 ...
 */
windows.logKeyTimings = function (that) {
    var times = that.model.state.keyboard.times;
    var maxRecords = that.model.config.keyboard.maxRecords || times.length;
