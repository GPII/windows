...
/**
 * Kills any windows processes with a given application filename using
 * the "killProcessByName.exe" binary file.
 *
...
gpii.windows.killProcessByName = function (filename) {
    ...
    exec(__dirname + "\\killProcessByName.exe " + filename, function (e, stdout) {
        if (e) {
            togo.reject({isError: true, message: e + "\n" + stdout});
        } else {
            togo.resolve(stdout);
        }
    });
