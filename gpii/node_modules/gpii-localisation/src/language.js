 ...
 *
 * These are listed in HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\MUI\UILanguages, however this has become
 * unreliable - so, only the languages explicitly installed via Morphic language installers will be used.
 *
 * Setting the environment variable GPII_DETECT_LANGUAGES will enable the original behaviour.
 *
 * The installers will write a value to HKLM\Software\Morphic\LanguageInstalled.
 *
...
gpii.windows.language.getInstalled = function (that) {
    ...
    var langCodes;
    if (process.env.GPII_DETECT_LANGUAGES) {
        langCodes = gpii.windows.enumRegistryKeys(
            "HKEY_LOCAL_MACHINE", "SYSTEM\\CurrentControlSet\\Control\\MUI\\UILanguages");
    } else {
        // Get the OS default language, because a language pack for that language isn't going to be installed.
        var LOCALE_NAME_MAX_LENGTH = 85;
        var codeBuffer = Buffer.alloc(LOCALE_NAME_MAX_LENGTH * 2);
        var result = gpii.windows.kernel32.GetSystemDefaultLocaleName(codeBuffer, codeBuffer.length);
        var osLang = result ? "en-US" : gpii.windows.stringFromWideChar(codeBuffer);

        var values = gpii.windows.enumRegistryValues("HKEY_LOCAL_MACHINE", "Software\\Morphic\\LanguageInstalled");
        if (values.statusCode) {
            langCodes = [osLang];
        } else {
            langCodes = fluid.keys(values);

            if (!langCodes.includes(osLang)) {
                langCodes.unshift(osLang);
            }
        }

        fluid.log("Installed Languages: " + osLang + " (OS), ", langCodes);
    }
