...
/**
 * Stops the Windows Explorer shell process, in a way that it can be restarted again so the folder windows are restored.
 *
 ...
 * this was called.
 *
 * @param {String} currentLanguage - The current (new) language.
 */
gpii.windows.updateLanguage = function (currentLanguage) {
    if (gpii.windows.updateLanguage.lastLanguage !== currentLanguage) {
        gpii.windows.updateLanguage.lastLanguage = currentLanguage;
        return gpii.windows.restartExplorer();
    }
};

gpii.windows.updateLanguage.lastLanguage =
    gpii.windows.readRegistryKey("HKEY_CURRENT_USER", "Control Panel\\Desktop", "PreferredUILanguages", "REG_SZ").value;
