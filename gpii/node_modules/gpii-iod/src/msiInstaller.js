...
gpii.windows.iod.msi.invokeMSI = function (that, installation, install) {
    ...
    if (install) {
        ...
        if (installation.packageData.uninstallerArgs) {
            args.push.apply(args, fluid.makeArray(installation.packageData.uninstallerArgs));
        }
    }
    ...
    if (installation.packageData.elevate) {
        promise = that.invokeElevated("msiexec.exe", args);
    } else {
        child_process.spawn("msiexec.exe", args);
    }
