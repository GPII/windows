...
jqUnit.asyncTest("Send zoom", function () {
    ...
    var nextTest = function () {
        ...
        if (currentTest) {
            ...
            // Set control key to a known state.
            gpii.windows.user32.keybd_event(gpii.windows.API_constants.VK_CONTROL, 0, 2, 0);
    ...
    appZoom = gpii.windows.appZoom({
        listeners: {
            ...
            "{gpii.windows.messages}.events.onMessage": function (hwnd, msg, wp, lp) {
                var handled = false;
                switch (msg) {
                ...
                if (handled) {
                    var ctrlDown = !!(gpii.tests.windows.appZoom.user32.GetKeyState(
                        gpii.windows.API_constants.VK_CONTROL) & 0x8000);
...
// Checks window activation is being detected.
jqUnit.asyncTest("Window activation", function () {
    ...
    appZoom.events.onApplicationActivated.addListener(function (that, window) {
        jqUnit.assertEquals("Activated window pid should be the child process", notepad.pid, window.pid);
        jqUnit.assertEquals("Activated window exe should be notepad", window.exe, "notepad.exe");
        jqUnit.assertNotNull("Activated window should have some configuration", window.config);
        jqUnit.assertTrue("Activated window should have correct configuration", window.config.correct);
        notepad.kill();
        jqUnit.start();
        process.nextTick(appZoom.destroy);
    });
