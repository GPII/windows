    ...
    for (var prop in expected) {
        if (expected.hasOwnProperty(prop)) {
            if (expected[prop] === undefined) {
                // "undefined" is the sentinel value for "It's expected, but don't care about the value."
                match = subject.hasOwnProperty(prop);
            } else if (fluid.isPrimitive(expected[prop])) {
                match = subject[prop] === expected[prop];
            } else {
                match = gpii.tests.metrics.deepMatch(subject[prop], expected[prop], maxDepth - 1);
            }
            if (!match) {
                break;
            }
