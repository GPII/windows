...
windows.display.getImpl = function (payload) {
    var results = {};

    if (fluid.get(payload, "settings.screen-resolution")) {
        var curRes = windows.display.getScreenResolution();
        results["screen-resolution"] = curRes;
    }

    if (fluid.get(payload, "settings.screen-dpi")) {
        var curDpi = windows.display.getScreenDpi();
        results["screen-dpi"] = curDpi.configured;
    }
