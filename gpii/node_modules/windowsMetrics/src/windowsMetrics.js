...
windows.startInputMetrics = function (that) {
    ...
    that.model.state.input.config = that.model.config.input;
};
...
windows.stopInputMetrics = function (that) {
    // remove the hooks
    if (that.model.state.input.keyboardHookHandle) {
        windows.user32.UnhookWindowsHookEx(that.model.state.input.keyboardHookHandle);
    }
    if (that.model.state.input.mouseHookHandle) {
        windows.user32.UnhookWindowsHookEx(that.model.state.input.mouseHookHandle);
    }
    that.model.state.input.keyboardHookHandle = null;
    that.model.state.input.keyboardHookCallback = null;
    that.model.state.input.mouseHookHandle = null;
    that.model.state.input.mouseHookCallback = null;
};
...
windows.recordKeyTiming = function (that, timestamp, specialKey) {
    var state = that.model.state.input;
        ...
        case windows.API_constants.WM_KEYUP:
            ...
            if ((eventData.flags & ignoreFlags) === 0) {
                ...
                if (eventData.vkCode === windows.API_constants.VK_LEFT) {
                    ...
                    wanted = that.model.state.input.lastKeyLogged;
                }
                if (wanted) {
                ...
                that.model.state.input.lastKeyLogged = wanted && !specialKey;
            }
            break;
