...
windows.NonClientMetrics = new Struct([
    ...
    [windows.logFontPointer,  "lfCaptionFont"],
    ["int32",    "iSmCaptionWidth"],
    ["int32",    "iSmCaptionHeight"],
    [windows.logFontPointer,  "lfSmCaptionFont"],
    ["int32",    "iMenuWidth"],
    ["int32",    "iMenuHeight"],
    [windows.logFontPointer,  "lfMenuFont"],
    [windows.logFontPointer,  "lfStatusFont"],
    [windows.logFontPointer,  "lfMessageFont"],
 ...
 */
windows.setFlagValues = function (payload) {
	/*var pvParam = windows.createEmptyStructure(structName);
	var combinedFlags = windows.combineFlags(flagNames);	
	
	pvParam.dwFlags = combinedFlags;
	*/
	var action = windows.actionConstants[payload.setAction];
	...
	if (!callSuccessful) {
		var errorCode = kernel32.GetLastError();
		console.log("setFlagValues() failed. Error code: " + errorCode);
		//fluid.fail("Could not set settings in structure " + payload.pvParam.name);
	}
	return pvParam;
};
...
windows.populateResults = function (payload, isNewValue, results) {
	var getAction = windows.actionConstants[payload.getAction];
	var uiParam = windows.getUiParam(payload);
	var structName = payload.pvParam.name;
	var settingsStruct = windows.getSettingsInStructure(getAction, uiParam, structName);
	
	//FIXME crashes when getting settingsStruct.lfFaceName - fixed?
	for (var property in payload.pvParam.value) {
		if (!isNewValue) {
			results[property] = {};
		}
		var valueToSet;
		var settingValue = payload.pvParam.value[property];
		/*if (property === FLAG_DATA_TYPE) {
			valueToSet = windows.getFlagValue(settingsStruct, currentSetting);
		} else {
			valueToSet = settingsStruct[currentSetting];
		}*/
		valueToSet = settingsStruct[property];
		if (isNewValue) {
			results[property].newValue = valueToSet;
		} else {
			results[property].oldValue = valueToSet;
		}
	}
...
windows.getUiParam = function (payload) {
	...
	if (uiParam === 'struct_size') {
		return windows.structures[payload.pvParam.name].size;
	}
...
windows.defineStruct = function (payload) {
	if (!windows.structures[payload.pvParam.name]) {
		if (!payload.pvParam.template) {
			throw 'Cannot create structure ' + payload.pvParam.name + ' - payload.pvParam.template not found!';
		}
		
		windows.structures[payload.pvParam.name] = new Struct(payload.pvParam.template);
	}
};

/**
 * before set, get requested settings from the payload
 */
windows.getPvParam = function (payload) {
	var pvParam = windows.createEmptyStructure(payload.pvParam.name);
	
	/*if(pvParam.dwFlags) {
		pvParam.dwFlags = windows.combineFlags(payload.pvParam.value.dwFlags);
	}*/
	...
	for (var property in payload.pvParam.value) {
		if(property === "dwFlags") {
			...
			if (typeof payload.pvParam.value[property] === "string") {
				pvParam[property] = util.windows.stringToWideChar(payload.pvParam.value[property]); 
			} else {
				pvParam[property] = payload.pvParam.value[property];
			}
		}
	}
...
gpii.windows.spiSettingsHandler = function (payload) {
	...
	windows.setFlagValues(payload);
	
	windows.populateResults(payload, true, results);
	
	console.log(results);
	
	//console.log('old value: ' + util.windows.fromWideChar(results.lpszDefaultScheme.oldValue).toString() + '.');
	//console.log('new value: ' + util.windows.fromWideChar(results.lpszDefaultScheme.newValue).toString() + '.');
