...
gpii.windows.closeProcessByName = function (filename, options) {
    ...
    var processWindows = gpii.windows.getProcessWindows(filename);

    var promiseTogo = fluid.promise();
    if (processWindows && processWindows.length > 0) {
...
/**
 * Gets the window handles owned by a process.
 *
 * @param {String|Number} process The PID or filename of the process.
 * @return {Array<Number>} An array of handles to the top level windows owned by the process(es), or null if the process
 *  does not exist.
 */
gpii.windows.getProcessWindows = function (process) {

    var pids;
    if (isNaN(process)) {
        pids = fluid.transform(processesBridge.findProcessesByCommand(process), function (proc) {
            return proc.pid;
        });
    } else {
        pids = [process];
 ...
 * @param {Boolean} options.activateCurrent true to activate the application's window if there's already an instance.
 *
