...
fluid.module.register("gpii.windows.spiSettingsHandlerModule", __dirname, require);
...
gpii.windows.spi.getCurrentSettings = function (payload) {
    ...
    if (payload.options.pvParam.type === "array") {
        if (payload.options.pvParam.valueType === "TCHAR") {
            var sBuf = pvParam.toString("utf16le");
            var nullPos = sBuf.indexOf("\0");
            currentSettings.pvParam = sBuf.substring(0, nullPos);
        } else {
            currentSettings.pvParam =
                gpii.windows.bufferToArray(pvParam, payload.options.pvParam.valueType);
        }
    } else {
        currentSettings.pvParam = pvParam.deref();
    }
...
gpii.windows.spi.systemParametersInfo = function (pvParamType, action, uiParam, pvParam, fWinIni) {
    ...
    fWinIni = typeof fWinIni !== "undefined" ? fWinIni : 0;
 ...
 * @param fWinIni The fWinIni SPI parameter.
...
gpii.windows.spi.checkWallpaperPath = function (path) {
    ...
    var isNotDir = false;
    try {
        isNotDir = !fs.lstatSync(path).isDirectory();
    } catch (e) {
        isNotDir = false;
    }

    var isJpg = path.endsWith(".jpg");
    var isPng = path.endsWith(".png");

    return isNotDir && isPng || isJpg;
};

gpii.windows.spi.checkWallpaperPayload = function (payload) {
    if (fluid.get(payload, "settings.Wallpaper.value") === undefined) {
        return false;
    }
    var path = payload.settings.Wallpaper.value;

    return gpii.windows.spi.checkWallpaperPath(path);
};
...
gpii.windows.spi.applySettings = function (payload) {
    ...
    var fWinIni = gpii.windows.resolveFlags(payload.options.fWinIni, gpii.windows.API_constants.SpiFlags);
    ...
    // We should include a verification step for the whole payload before
    // converting the arguments with the 'get**Param' functions.
    // if (action === gpii.windows.actionConstants.SPI_SETDESKWALLPAPER) {
    //     var validPayload = gpii.windows.spi.checkWallpaperPayload(payload);
    //     if (!validPayload) {
    //         promiseTogo.reject({
    //             isError: true,
    //             message: "gpii.windows.spi.applySettings: Invalid PvParam payload '" + pvParam + "'"
    //         });
    //         return promiseTogo;
    //     }
    // }
        ...
        .then(function () {
            ...
            if (sendChange || (problematicSpiCalls.indexOf(action) >= 0)) {
                ...
                if (callSuccessful) {
                    ...
                    fluid.log("Something get wrong");
                    var errorCode = gpii.windows.kernel32.GetLastError();
                    promiseTogo.reject({
                        isError: true,
                        message: "SpiSettingsHandler.js: spi.applySettings() failed with error code " + errorCode + "."
                    });
                }
            }
        });
...
/**
 * @param {String} s Returns true if value is a string, false otherwhise.
 */
function isString(s) {
...
/**
 * @param {*} s String to count its words.
 * @return The number of words in the string parameter.
 */
...
/**
 * @param {*} input String to be splitted into words.
 */
...
/**
 * @param {String} flags String with the flags to be parsed.
 * @return The values of the parsed string.
 */
gpii.windows.spi.parseFWinIniFlags = function (flags) {
    if (flags === undefined || !isString(flags)) {
        fluid.log("SpiSettingsHandler.js: spi.parseWinIni() got invalid flags param: " + flags + " of type " + typeof flags + ".");
        return 0;
    } else {
        if (countWords(flags) > 1) {
            var words = splitInWords(flags);
            var pFlags = [];
            ...
            while (words.length >= 1) {
                ...
                if (fWord !== "|") {
                    var flag = gpii.windows.fWinIniFlags[fWord];

                    if (flag !== undefined) {
                        pFlags.push(flag);
                    } else {
                        fluid.log("SpiSettingsHandler.js: spi.parseWinIni() got invalid flags param: " + fWord + " of type " + typeof fWord + ".");
                        return 0;
                    }
                }
            }

            // Linter (Bug) Workaround: Linter fails with arrow style lambda with multiple parameters.
            var value = pFlags.reduce(function (v1,v2) {
                return v1 | v2;
            });
...
gpii.windows.spi.getFWinIni = function (payload) {
    var fWinIni = payload.options.fWinIni;
    var flagValue = gpii.windows.spi.parseFWinIniFlags(fWinIni);

    if (flagValue === undefined) {
        fluid.log("SpiSettingsHandler.js: spi.getFWinIni() got unknown fWinIni param value: " + fWinIni + " of type " + typeof fWinIni + ".");
        return 0;
    } else {
        return flagValue;
    }
