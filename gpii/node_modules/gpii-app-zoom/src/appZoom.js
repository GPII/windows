...
windows.appZoom.sendZoom = function (that, direction) {
    ...
    if (config) {
        ...
        if (config.wheel) {
            setTimeout(windows.appZoom.sendWheel, config.wheel.delay || 0, hwnd, increment, config.wheel.simulate);
 ...
 * @param {Boolean} simulate true to move the mouse pointer over the window, and simulate the wheel event.
 */
windows.appZoom.sendWheel = function (hwnd, direction, simulate) {
    var MK_CONTROL = 0x8;
    var WHEEL_DELTA = 120;
