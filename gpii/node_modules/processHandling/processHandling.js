 ...
 * @param options.start The desired state: true to wait for the process to start, false to wait for the termination, or
 *   null to wait for a change (if the process isn't running then wait for it to start, otherwise wait for termination).
 ...
 */
gpii.windows.waitForProcessStateChange = function (filename, options) {
    ...
    var checkProcess = function () {
        ...
        var running = windows.findProcessByName(filename) !== null;
        var changed = false;

        if (options.start === null) {
            // Auto-detect the mode based on whether or not it's currently running.
            options.start = !running;
        } else if (running === options.start) {
            changed = true;
        }
        ...
        if (changed) {
            ...
            if (!options.timeout || Date.now() < timeoutTime) {
                // It's still in the same state - keep checking.
                setTimeout(checkProcess, options.pollDelay);
            } else {
                // Ran out of time.
                promise.reject({
                    isError: true,
 ...
 */
gpii.windows.waitForProcessTermination = function (filename, options) {
    options = options || {};
    options.start = false;
    return gpii.windows.waitForProcessStateChange(filename, options);
};
 ...
 */
gpii.windows.waitForProcessStart = function (filename, options) {
    options = options || {};
    options.start = true;
    return gpii.windows.waitForProcessStateChange(filename, options);
};
 ...
 *
 * If options.terminate is true (default), and if the process has not shutdown after the timeout or the process
 ...
 * @param options.terminate true to force the termination if the process can't be closed cleanly; false to reject.
 * @return {promise}
 */
gpii.windows.closeProcessByName = function (filename, options) {
    ...
    // Enumerate all the top-level windows on the desktop.
    gpii.windows.enumerateWindows(function (hwnd) {
        ...
        if (pids.indexOf(windowPid) !== -1) {
            ...
            gpii.windows.user32.PostMessageW(hwnd, windows.API_constants.WM_QUIT, 0, 0);
            foundWindow = true;
        }
    });
