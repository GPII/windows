...
jqUnit.test("Enumerating registry key values", function () {
    ...
    jqUnit.expect(testData.initPayload.count * 4 + 1);

    // Apply the data
    gpii.windows.registrySettingsHandler.setImpl(testData.initPayload);

    // Enumerate the key
    var result = gpii.windows.enumRegistryValues(baseKey, path, function (name, data, type) {
        jqUnit.assertNotUndefined("Checking callback", testData.expectedReturn[name]);
        jqUnit.assertEquals("Checking name from callback", testData.expectedReturn[name].name, name);
        jqUnit.assertEquals("Checking data from callback", testData.expectedReturn[name].value, data);
        jqUnit.assertEquals("Checking type from callback", testData.expectedReturn[name].type, type);
    });
