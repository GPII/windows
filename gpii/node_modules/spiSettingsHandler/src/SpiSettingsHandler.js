...
windows.LogFont.size = 92; // FIXME - magic number here
...
windows.getSettingsInStructure = function (action, uiParam, structName) {
	...
	//FIXME returns false when called with NONCLIENTMETRICS
...
windows.populateResults = function (payload, isNewValue, results) {
	...
	for (var property in payload.settings) {
		...
		var valueToSet;
		var settingValue = payload.settings[property];
...
windows.defineStruct = function (payload) {
	if (!windows.structures[payload.options.pvParam.name]) {
...
windows.getPvParam = function (payload) {
	...
	//currently working IF pvParam.type === 'struct'
	
	//var pvParam = windows.createEmptyStructure(payload.options.pvParam.name);
	var pvParam = windows.getSettingsInStructure(windows.actionConstants[payload.options.getAction], windows.getUiParam(payload), payload.options.pvParam.name);
	...
	var setField = function (struct, field, value) {
		...
		if (valueType === 'number' || valueType === 'boolean') {
			...
			struct[field] = util.windows.stringToWideChar(value);
		}
	...
	/*for (var property in payload.settings) {
		if(property === "dwFlags") {
			pvParam.dwFlags = windows.combineFlags(payload.settings.dwFlags);
		}
		else {
			if (typeof payload.settings[property] === "string") {
				pvParam[property] = util.windows.stringToWideChar(payload.settings[property]); 
			} else {
				pvParam[property] = payload.settings[property];
			}
		}
	}*/
...
gpii.windows.spiSettingsHandler = function (payload) {
	...
	//console.log(results.lfMessageFont.newValue.lfQuality)
