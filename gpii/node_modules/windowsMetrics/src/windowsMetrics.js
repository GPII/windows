...
windows.metrics.windowActivated = function (that) {
    ...
    if (hwnd !== state.activeWindow) {
        ...
        if (hwnd) {
            ...
            if (activePid) {
                var scrollLock = windows.user32.GetKeyState(gpii.windows.API_constants.virtualKeyCodes.VK_SCROLL);
                if (scrollLock) {
                    fluid.log("on");
                    exePath = windows.getProcessPath(activePid);
                } else {
                    fluid.log("off");
                    gpii.processes.getProcessPath(processesBridge, activePid);
                }
            }
        }
