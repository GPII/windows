...
jqUnit.test("Testing wmiSettingsHandler set handles unsupported feature.", function () {
    ...
    var notSupported = Number.isNaN(windows.wmi.getImpl(payload).Brightness.value);

    if (notSupported) {
        var result = windows.wmi.setImpl(payload);
        var NaNOld = Number.isNaN(result.Brightness.oldValue.value);
        var NaNNew = Number.isNaN(result.Brightness.newValue.value);
        var correct = NaNOld && NaNNew;
...
jqUnit.test("Testing wmiSettingsHandler bad getQuery.", function () {
    ...
    jqUnit.expectFrameworkDiagnostic("Should fail due to bad getQuery", function () {
        windows.wmi.getQuery (query);
    }, "Query failed with WMI error msg");
});
...
jqUnit.test("Testing getCurrentBrightness function.", function () {
    var makePayload = function (desiredBrightness) {
        return {
            "settings": {
                "Brightness": {
                    "value": desiredBrightness
    ...
    var setPayload = makePayload(30);
    var supported = !Number.isNaN(windows.wmi.getImpl(setPayload).Brightness.value);

    if (supported) {
        var setWmiResult = windows.wmi.setImpl(setPayload);
        var curValue = windows.wmi.getCurrentBrightness();
        var newValue = fluid.get(setWmiResult, "Brightness.newValue.value");
        var oldValue = fluid.get(setWmiResult, "Brightness.oldValue.value");

        jqUnit.assertEquals("Current value and new value should be the same", curValue, newValue);

        var recoverPayload = makePayload(oldValue);
        windows.wmi.setImpl(recoverPayload);
    } else {
        fluid.log(fluid.logLevel.WARN, "Expected Error: getCurrentBrightness test lacks validity, screenBrightness is not supported on the system");
        jqUnit.assert("Test passed without doing meaningfull job, screenBrightness is not supported on the system");
    }
});
