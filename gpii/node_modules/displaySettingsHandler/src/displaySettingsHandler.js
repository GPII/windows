...
if (ver >= "10") {
    // Windows 10
    require("./dpi_win10.js");
} else if (ver >= "6.2") {
    // Windows 8
} else {
    // Windows 7 (or older): DPI changes unsupported - requires reboot.
}
...
windows.display.setImpl = function (payload) {
    ...
    var dpi = payload.settings["screen-dpi"];
    if (dpi) {
        var oldDpi = windows.display.getScreenDpi();
        windows.display.setScreenDpi(dpi);
        // set dpi
        results["screen-dpi"] = { oldValue: oldDpi, newValue: dpi };
    }
