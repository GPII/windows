...
var testPayloads = ["testMouseTrails", "testFilterKeys", "testMouse", "testMouseClickLock", "testLogFont",
    // Commented out for GPII-2001 - test now stall extremely often
    // "testNonClientMetrics",
    "testStickyKeys", "testHighContrast", "testHighContrastToEmpty", "testMouseKeys", "testToggleKeys",
    "testUnderlineShortcuts", "testAudioDescription", "testNotificationDuration"];
...
jqUnit.asyncTest("Testing setHighContrastTheme", function () {

    var tests = fluid.transform(highContrastThemes, function (test) {
        return function () {

            var file = test.file.replace("%SystemRoot%", process.env.SystemRoot);
            var promise = gpii.windows.spiSettingsHandler.setHighContrastTheme(file, null, null, true);

            promise.then(function (result) {
                jqUnit.assertEquals("setHighContrastTheme must resolve with the expected result, file=" + file,
                    test.expect, result);
            });
    ...
    fluid.promise.sequence(tests).then(jqUnit.start, fluid.fail);
});
