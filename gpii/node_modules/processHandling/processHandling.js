...
gpii.windows.closeCommandWithRetry = function (promise, command, args, processToWait, processToClose, options, retryNum) {
    var closeCommand = gpii.launch.spawn(command, args);

    closeCommand.on("exit", function () {
        gpii.windows.waitForProcessTermination(processToWait, {timeout: 200, pollDelay: 100})
            .then(function () {
                gpii.windows.closeProcessByName(processToClose, {cleanOnly: false, timeout: 150})
                    .then(function () {
                        if (options.restart) {
                            ...
                            child_process.execFile(b64Cmd, ["/k", processToClose]);
            ...
            function () {
                fluid.log("gpii.windows.closeCommandWithRetry: Retry number " + retryNum + ".");
                if (options.retries !== 0) {
                    gpii.windows.closeCommandWithRetry(promise, command, args, processToWait, processToClose, options, retryNum + 1);
                } else {
                    fluid.log("gpii.windows.closeCommandWithRetry: Closing failed after " + retryNum + "retries.");
                }
            });
    });

    closeCommand.on("error", function (error) {
        promise.reject({
            isError: true,
...
/**
 * @param command The command to be executed for closing an application.
 * @param args The arguments that are going to be passed to the command.
 * @param processToWait The process that is going to be closed when the command executes.
 * @param processToClose The process for closing after the previous one has exited.
 * @param options.timeout How long to wait for the process to die, in milliseconds.
 * @param options.cleanOnly true to reject if the process can't be closed cleanly; false to force the termination.
 * @param options.exitCode {number} The exit code the application should return.
 ...
 * @return {promise}
 */
gpii.windows.closeAfterExitCommand = function (command, args, processToWait, processToClose, options) {
    var defaultOptions = {
        timeout: 15000,
        cleanOnly: false,
        exitCode: 0
    ...
    gpii.windows.closeCommandWithRetry(promise, command, args, processToWait, processToClose, options, 0);

    return promise;
};

/**
 * @param command The command to be executed for opening an application.
 * @param args The arguments that are going to be passed to the command.
 * @param processToWait Process that is going to be opened when the command executes.
 * @param processToClose Process for closing after the previous one has exited.
 * @param options.timeout How long to wait for the process to die, in milliseconds.
 * @param options.cleanOnly true to reject if the process can't be closed cleanly; false to force the termination.
 * @param options.exitCode {number} The exit code the application should return.
 * @param options.restart Restart the processToClose instead of just closing it.
 * @return {promise}
 */
gpii.windows.closeAfterOpenCommand = function (command, args, processToWait, processToClose, options) {
    var defaultOptions = {
        timeout: 15000,
        cleanOnly: false,
        exitCode: 0
    ...
    var promise = fluid.promise();
    gpii.windows.waitForProcessStart(processToWait, {pollDelay: 100}).then(function () {
        gpii.windows.waitForProcessStart(processToClose, {pollDelay: 100}).then(function () {
            gpii.windows.closeProcessByName(processToClose, {cleanOnly: false, timeout: 150})
                .then(function () {
                    fluid.log("Restart value: " + options.restart);
                    if (options.restart) {
                        var b64Cmd = process.env.windir + "\\sysnative\\cmd.exe";
                        child_process.execFile(b64Cmd, ["/k", processToClose]);
                        promise.resolve(true);
    ...
    var openCommand = gpii.launch.spawn(command, args);

    openCommand.on("error", function (error) {
        promise.reject({
            isError: true,
            message: "Error while executing command '" + command + "' with args '" + "'args'",
            error: error
...
fluid.defaults("gpii.windows.closeAfterExitCommand", {
    gradeNames: "fluid.function",
    argumentMap: {
        command: 0,
        args: 1,
...
fluid.defaults("gpii.windows.closeAfterOpenCommand", {
    gradeNames: "fluid.function",
    argumentMap: {
        command: 0,
        args: 1,
