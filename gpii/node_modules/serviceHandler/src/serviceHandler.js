...
var messaging = fluid.require("%gpii-windows/service/shared/pipe-messaging.js");
...
/**
 * Example request.
 * @param that {Component} The gpii.serviceHandler instance.
...
gpii.windows.service.connectToService = function (that) {
    ...
    if (pipeId) {
        ...
        promise = gpii.windows.service.serviceAuthenticate(that, pipeName).then(function (data) {
            ...
            that.session = messaging.createSession(that.pipe, "gpii", that.handleRequest, data);
 ...
 *
 * @param that {Component} The gpii.serviceHandler instance.
 * @param request {Object} The request. It should at least contain "name" which is the name of the request.
 */
gpii.windows.service.handleRequest = function (that, request) {
    fluid.log("Service: request", request);
    if (that.requestHandler.options.invokers.hasOwnProperty(request.name)) {
        return that.requestHandler[request.name](request);
    }
};

if (!process.env.GPII_SERVICE_PIPE_DISABLED) {
    process.nextTick(gpii.windows.service.serviceHandler);
}
