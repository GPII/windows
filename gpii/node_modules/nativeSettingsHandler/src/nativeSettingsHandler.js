 ...
 * is malfunctioning.
 *
 * @param {Boolean} bool A boolean representing the desired state for the 'screen mirror' feature.
 * @return {Promise} A promise that resolves on success or holds a error code and message on failure.
 */
windows.nativeSettingsHandler.SetScreenMirrored = function (bool) {
    var pRes = fluid.promise();
    var err = 0;

    if (bool) {
        var cloneFlags = windows.API_constants.SDC_TOPOLOGY_CLONE | windows.API_constants.SDC_APPLY;
        err = gpii.windows.user32.SetDisplayConfig(0, ref.NULL, 0, ref.NULL, cloneFlags);
    } else {
        var internalFlags = windows.API_constants.SDC_TOPOLOGY_INTERNAL | windows.API_constants.SDC_APPLY;
        err = gpii.windows.user32.SetDisplayConfig(0, ref.NULL, 0, ref.NULL, internalFlags);
    }
...
windows.nativeSettingsHandler.GetScreenMirrored = function () {
    ...
    var DISPLAYCONFIG_PATH_INFO_SIZE = 62;
    var DISPLAYCONFIG_MODE_INFO_SIZE = 74;
