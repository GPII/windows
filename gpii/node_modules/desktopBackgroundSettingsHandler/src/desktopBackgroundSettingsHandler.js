...
windows.desktopBackground.checkBackgroundType = function (payload) {
    var result = false;
...
windows.desktopBackground.addBackgroundType = function (request, mode) {
    if (mode === "get") {
...
windows.desktopBackground.addSolidColor = function (request, mode) {
    if (mode === "set") {
        request.settings[windows.desktopBackground.revIdMaps.BackgroundType] = {
            value: windows.desktopBackground.sysPayloadValues.SolidColor
        };
    } else {
        request.settings[windows.desktopBackground.revIdMaps.BackgroundType] = {};
    }
...
windows.desktopBackground.addPicture = function (request, mode, payload) {
    if (mode === "set") {
        request.settings[windows.desktopBackground.revIdMaps.BackgroundType] = {
            value: "Picture"
        };
    ...
    if (fluid.get(payload, "Input.value.Scaling") !== undefined) {
        if (mode === "set") {
            request.settings[windows.desktopBackground.revIdMaps.BackgroundScaling] = {
...
windows.desktopBackground.setDeskWallpaper = function (picturePath) {
    ...
    if (!windows.desktopBackground.checkPicturePath(picturePath)) {
 ...
 *
 * @return {Promise} A promise with the current wallpaper value.
 */
...
/**
 * This specific setting behaves in a incorrect way where trying to set it fromt he systemSettingsHandler. So,
 * to keep the uniform way of dealying with payloads, we remove it after creating the request for the
 * systemSettingsHandler.
 *
 * @param {Object} request The request from which remove the invalid request.
 ...
 *  to be build.
 */
windows.desktopBackground.removeInvalidRequests = function (request, mode, payload) {
    if (windows.desktopBackground.isPicture(payload)) {
        if (mode === "set") {
            delete request.settings[windows.desktopBackground.revIdMaps.BackgroundType];
        }
    }
};
...
windows.desktopBackground.buildSystemSettingsRequest = function (payload, mode) {
    var requestCheckers = [
        ["BackgroundType", windows.desktopBackground.isBackgroundType, windows.desktopBackground.checkBackgroundType, windows.desktopBackground.addBackgroundType],
        ["SolidColor", windows.desktopBackground.isSolidColor, windows.desktopBackground.checkSolidColorInput, windows.desktopBackground.addSolidColor],
        ["Picture", windows.desktopBackground.isPicture, windows.desktopBackground.checkPictureInput, windows.desktopBackground.addPicture],
        ["Slideshow", windows.desktopBackground.isSlideShow, windows.desktopBackground.checkSlideShowInput, windows.desktopBackground.addSlideshow]
    ];
    ...
    for (var index = 0; index < requestCheckers.length; index++) {
        ...
        if (checker[1](payload)) {
            if (checker[2](payload)) {
                request = checker[3](request, mode, payload);
                break;
            } else {
    ...
    windows.desktopBackground.removeInvalidRequests(request, mode, payload);
...
windows.desktopBackground.mapBackgroundType = function (sysResults, mode) {
    ...
    if (mode === "get") {
        ...
        if (backgroundTypeVal === windows.desktopBackground.sysPayloadValues.SolidColor) {
            fluid.set(backgroundType, "value", "SolidColor");
        } else if (backgroundTypeVal === undefined ) {
            // There is a unexpected behavior in triggering the background Picture setting. Which
            // is why this setting has been facked in this request mappings.
            fluid.set(backgroundType, "value", "Picture");
        } else {
        ...
        if (oldBackgroundTypeVal === undefined) {
            // There is a unexpected behavior in triggering the background Picture setting. Which
            // is why this setting has been facked in this request mappings.
            fluid.set(backgroundType, "oldValue.value", "Picture");
        }
        if (newBackgroundTypeVal === undefined) {
            // There is a unexpected behavior in triggering the background Picture setting. Which
            // is why this setting has been facked in this request mappings.
            fluid.set(backgroundType, "newValue.value", "Picture");
        }
        if (oldBackgroundTypeVal === windows.desktopBackground.sysPayloadValues.SolidColor) {
            fluid.set(backgroundType, "oldValue.value", "SolidColor");
        } else {
            fluid.set(backgroundType, "oldValue.value", oldBackgroundTypeVal);
        }
        if (newBackgroundTypeVal === windows.desktopBackground.sysPayloadValues.SolidColor) {
...
windows.desktopBackground.mapInput = function (input, sysResults, backgroundType, path) {
    ...
    if (input[1] === undefined) {
        ...
        } else if (backgroundTypeVal === undefined || backgroundTypeVal === "Picture") { // "Picture") {
            if (fluid.get(sysResults, windows.desktopBackground.revIdMaps.BackgroundScaling + "." + path) !== undefined) {
                fluid.set(backgroundType, path, "Picture");
                var scaling = fluid.get(sysResults[windows.desktopBackground.revIdMaps.BackgroundScaling], path);
 ...
 */
windows.desktopBackground.addHandlerResults = function (results, values, backgroundTypeVal, path) {
    if (backgroundTypeVal === "SolidColor") {
 ...
 */
windows.desktopBackground.addInputVals = function (values, results, mode) {
    if (mode === "get") {
        var backgroundTypeVal = fluid.get(results, "BackgroundType.value");
        results = windows.desktopBackground.addHandlerResults(results, values.values, backgroundTypeVal, "Input.value");
    } else {
        ...
        results = windows.desktopBackground.addHandlerResults(results, values.oldValues, oldBackgroundTypeVal, "Input.oldValue.value");
        results = windows.desktopBackground.addHandlerResults(results, values.newValues, newBackgroundTypeVal, "Input.newValue.value");
    }

    return results;
};

windows.desktopBackground.processSolidColorPayload = function (pRes, wallRes, payload, handlerValues, sysResults, curMode) {
    ...
    if (sColorRes === 0) {
        ...
        var colorResults = windows.desktopBackground.addInputVals(handlerValues, sysResults, curMode);

        pRes.resolve(colorResults);
    }
};

windows.desktopBackground.processPicturePayload = function (pRes, wallRes, payload, handlerValues, sysResults, curMode) {
    var picture = fluid.get(payload, "Input.value.Image");
    var validPicture = windows.desktopBackground.checkPicturePath(picture);
    ...
    if (validPicture) {
        ...
        wallPr.then(
            function (spiResults) {
                fluid.set(handlerValues, "oldValues.image", wallRes.value);
                fluid.set(handlerValues, "newValues.image", spiResults.newValue);

                var picResults = windows.desktopBackground.addInputVals(handlerValues, sysResults, curMode);
...
windows.desktopBackground.setImpl = function (payload) {
    ...
    var sysRequest = windows.desktopBackground.buildSystemSettingsRequest(payload, curMode);
    ...
    if (sysRequest[1] !== undefined) {
        ...
        var curWallpaper = windows.desktopBackground.getDeskWallpaper();
        var handlerValues = { oldValues: { color: curSysColor, image: curWallpaper.value } };
        ...
        pSysResults.then(function (rawSysResults) {
            ...
            if (sysResults[1] !== undefined) {
                ...
                if (windows.desktopBackground.isSolidColor(payload)) {
                    windows.desktopBackground.processSolidColorPayload(pRes, curWallpaper, payload, handlerValues, sysResults, curMode);
                } else if (windows.desktopBackground.isPicture(payload)) {
                    windows.desktopBackground.processPicturePayload(pRes, curWallpaper, payload, handlerValues, sysResults, curMode);
                } else {
...
windows.desktopBackground.getImpl = function (payload) {
    ...
    if (sysRequest[1] !== undefined) {
        ...
        var pSysResults = windows.desktopBackground.getSystemSettings(sysRequest);
        ...
        pSysResults.then(
            function (rawSysResults) {
                ...
                if (sysResults[1] !== undefined) {
                    ...
                    var curSysColor = windows.getSolidColor();
                    var curWallpaper = windows.desktopBackground.getDeskWallpaper();
                    var handlerValues = { values: { color: curSysColor, image: curWallpaper.value } };

                    var results = windows.desktopBackground.addInputVals(handlerValues, sysResults, curMode);
