 ...
 * @param {String} siteSpecific [optional] Field name in the service's secrets file that is specific to the site.
...
gpii.windows.userListeners.getUserId = function (userIdSource) {
    ...
    if ((source === "disable") || (source === "disabled")) {
        ...
        // A registry location
        if (source === "regkey") {
            userIdSource += ":HKCU\\SOFTWARE\\Morphic\\gpiiKey";
        }
        // Parse the value, breaking it into the root registry key, path, and value name.
        // For example, "HKLM\path\path\value"
        ...
        // A file
        var path = gpii.windows.expandEnvironmentStrings(source.substr(5));
    ...
    var togo = null;
    if (userId !== "" && fluid.isValue(userId)) {
        togo = (source === "userid") ? userId : userId.toLowerCase();
    }
    return togo;
