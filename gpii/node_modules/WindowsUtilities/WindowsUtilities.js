...
/**
 * Gets the icon cell size, which is used by explorer to space icons.
 * @return {Object} containing the x and y sizes of icon cells, in pixels.
 */
gpii.windows.getIconSpacing = function () {
    var iconSpacing = {};
    var intBuffer = ref.alloc("int");

    var oldDpiAwareness = gpii.windows.user32.SetThreadDpiAwarenessContext(-1 >>> 0);
    try {
        gpii.windows.spi.systemParametersInfoWPtr.SystemParametersInfoW(
            gpii.windows.spi.actions.SPI_ICONHORIZONTALSPACING, 0, intBuffer, 0);
        iconSpacing.x = intBuffer.deref();

        gpii.windows.spi.systemParametersInfoWPtr.SystemParametersInfoW(
            gpii.windows.spi.actions.SPI_ICONVERTICALSPACING, 0, intBuffer, 0);
        iconSpacing.y = intBuffer.deref();
    } finally {
        gpii.windows.user32.SetThreadDpiAwarenessContext(oldDpiAwareness);
    }
...
/**
 * Sets the icon cell size.
 * @param {Object} iconSpacing The icon cell size
 * @param {Number} iconSpacing.x The width, in pixels.
 * @param {Number} iconSpacing.y The height, in pixels.
 * @param {Boolean} update True to broadcast an update.
 */
gpii.windows.setIconSpacing = function (iconSpacing, update) {
    var fWinIni = gpii.windows.API_constants.SpiFlags.SPIF_UPDATEINIFILE;

    var oldDpiAwareness = gpii.windows.user32.SetThreadDpiAwarenessContext(-1 >>> 0);
    gpii.windows.spi.systemParametersInfoWUint.SystemParametersInfoW(gpii.windows.spi.actions.SPI_ICONHORIZONTALSPACING,
        iconSpacing.x, 0, fWinIni);

    if (update) {
        // Send the update on the last one, to cause a single refresh.
        fWinIni |= gpii.windows.API_constants.SpiFlags.SPIF_SENDCHANGE;
    }

    gpii.windows.spi.systemParametersInfoWUint.SystemParametersInfoW(gpii.windows.spi.actions.SPI_ICONVERTICALSPACING,
        iconSpacing.y, 0, fWinIni);
    gpii.windows.user32.SetThreadDpiAwarenessContext(oldDpiAwareness);
};
