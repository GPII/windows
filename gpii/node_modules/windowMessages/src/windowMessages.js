 ...
 *
 * @param {Number|String} window Window identifier. Can be the window handle, class name, or "BROADCAST"/null.
...
gpii.windows.messages.sendData = function (that, window, number, data) {
    ...
    var dataBuf = (typeof(data) === "string") ? windows.stringToWideChar(data) : data;
    if (!fluid.isApplicable(dataBuf.address)) {
        fluid.fail("sendData: data must be a string or ref buffer");
    }
    copyData.cbData = dataBuf.length;
    copyData.dwData = number;
    copyData.lpData = dataBuf;

    gpii.windows.messages.sendMessage(that.getWindowHandle(), windows.API_constants.WM_COPYDATA, that.getWindowHandle(),
