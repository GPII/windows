...
gpii.windows.enableRegisteredAT = function (name, enable, options) {
    ...
    if (enable && !options.configOnly) {
        ...
        gpii.windows.whilePendingAT().then(function () {
            ...
            gpii.windows.writeRegistryKey(gpii.registeredAT.accessibilityTempKey.baseKey,
                gpii.registeredAT.accessibilityTempKey.path, name, targetValue, "REG_DWORD");
            // Invoke utilman to trigger the de-registration.
            gpii.windows.nativeExec(options.utilman);
            promise.resolve();
        });
    }
 ...
 *
 * @return {promise} Returns a promise, whose value is true upon timeout, resolving when there are no pending actions.
 */
gpii.windows.whilePendingAT = function (options) {
    ...
    var checkActions = function () {
        ...
        var found = false;
        gpii.windows.enumRegistryValues(gpii.registeredAT.accessibilityTempKey.baseKey,
            gpii.registeredAT.accessibilityTempKey.path, function (name, data) {
                if ((data === enableAT) || (data === enableAT)) {
                    found = true;
                    return true;
                }
            });
 ...
 * @return {*} Returns what child_process.exec returns.
 */
gpii.windows.nativeExec = function (command, options) {
    var execOptions = fluid.extend({}, options);
    if (gpii.windows.isWow64()) {
        execOptions.shell = process.env.SYSTEMROOT + "\\Sysnative\\cmd.exe";
    }
    return child_process.exec(command, execOptions);
};
...
gpii.windows.getATInformation = function (name, values) {
    var regPath = gpii.registeredAT.registeredATKey.path + "\\" + name;
    var togo = {};
    for (var n = 0, len = values.length; n < len; n++) {
        var value = gpii.windows.readRegistryKey(gpii.registeredAT.registeredATKey.baseKey, regPath, values[n], "REG_SZ");
