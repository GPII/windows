...
windows.metrics.windowMessage = function (that, hwnd, msg, wParam, lParam) {
    switch (msg) {
    case that.WM_SHELLHOOKMESSAGE:
        var hwndTarget = (lParam && lParam.address) ? lParam.address() : lParam || 0;
        // Run the code in the next tick so this function can return soon, as it's a window procedure.
        process.nextTick(windows.metrics.shellMessage, that, wParam, hwndTarget);
        break;

    default:
        if (windows.metrics.settingsMessages.indexOf(msg) > -1) {
            process.nextTick(windows.metrics.configMessage, that, hwnd, msg, wParam, lParam.address());
        }
        break;
    }
};
...
windows.metrics.getWindowInfo = function (hwnd) {
    ...
    if (windowInfo.pid) {
        windowInfo.exe = processesBridge.getProcessPath(windowInfo.pid);
    } else {
        windowInfo.pid = 0;
    }
...
/**
 * Takes a path and attempt to generic looking path which is prefixed with a well-known environment variable instead of
 * the real path.
 *
...
windows.metrics.genericisePath = function (rawPath, env) {
    ...
    var envFound = fluid.find(envPaths, function (envName) {
        if (env[envName]) {
