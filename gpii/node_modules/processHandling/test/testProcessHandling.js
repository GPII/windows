...
jqUnit.test("Testing getProcessWindows", function () {
    ...
    // Get the "explorer.exe" windows (identify the process by the name). There may be other processes by that name,
    // so the list from the pid will be a subset of this.
    var namedExplorerWindows = gpii.windows.getProcessWindows("explorer.exe");
    var allFound = explorerWindows.every(function (w) {
        return namedExplorerWindows.includes(w);
    });
    jqUnit.assertTrue("Windows from explorer pid should be in the named process window list", allFound);
    ...
    // non-running process
    var notRunningWindows = gpii.windows.getProcessWindows("not a process");
    jqUnit.assertEquals("getProcessWindows(unknown pid) should return null", null, notRunningWindows);
});
