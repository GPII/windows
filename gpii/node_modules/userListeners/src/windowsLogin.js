 ...
 *
 * The key is generated using the signed digest of the user's SID. This will ensure that a user's key can't be guessed
 * by just knowing the sid, without also knowing the signing key.
 *
 * @param {Component} that The gpii.windows.userListeners.proximity instance.
 * @param {Function} sign The function used to sign the user id.
 * @return {Promise} Resolves when the listener has started.
 */
...
gpii.windows.getUserSid = function () {
    // Lazily initialise the .NET function.
    if (!gpii.windows.GetUserSid) {
        gpii.windows.GetUserSid = edge.func({
    ...
    return gpii.windows.GetUserSid(null, true);
};
