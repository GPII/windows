...
fluid.defaults("gpii.windows.service.clientCredentialsSource", {
    ...
    invokers: {
        get: {
            ...
            args: ["{gpii.windows.service.serviceHandler}"]
        }
    }
});

/**
 * Gets the client credentials from the service's secrect file.
 * @param {Component} service The service handler instance.
 * @return {Promise} Resolves with the client credentials.
 */
gpii.windows.service.getClientCredentials = function (service) {
    var promise = fluid.promise();
    service.requestSender.getSecret().then(function (secret) {
        if (secret && secret.clientCredentials) {
            promise.resolve(secret.clientCredentials);
        } else {
            promise.reject({
                isError: true,
                message: "Service returned no secret"
            });
        }
    }, promise.reject);
    return promise;
