...
windows.openRegistryKey = function (baseKey, path, keyHolder, daclAccess) {
    ...
    if (daclAccess) {
        code = advapi32.RegOpenKeyExW(base, pathW, 0, regPath.desiredAccess | windows.accessTypes.STANDARD_RIGHTS_REQUIRED, keyHolder);
    } else {
        code = advapi32.RegOpenKeyExW(base, pathW, 0, regPath.desiredAccess | c.KEY_QUERY_VALUE, keyHolder);
    }
...
windows.getSecurityDescriptorDacl = function (secBuffer, isPresent, isDefaulted) {
    ...
    if (desDACLCode === 0) {
        fluid.fail("Error: Failed to get security descriptor DACL.");
    } else {
        return oldACL;
    }
};
...
windows.setSecurityDescriptorDacl = function (secBuffer, daclPresent, aclRef, daclDefaulted) {
    ...
    if (resCode === 0) {
        fluid.fail("Error: Failed to set security descriptor ACL.");
    }
};
...
windows.writeRegistryKey = function (baseKey, path, subKey, value, type, uwpKey) {
    ...
    if (uwpKey) {
        code = advapi32.RegCreateKeyExW(
            base, pathW, 0, NULL, 0,
            regPath.desiredAccess | c.KEY_SET_VALUE | windows.accessTypes.STANDARD_RIGHTS_REQUIRED, NULL, keyHolder, NULL
      );
    } else {
        code = advapi32.RegCreateKeyExW(
            base, pathW, 0, NULL, 0,
    ...
    try {
        ...
        if (uwpKey) {
            ...
            // This variable should be free using LocalFree
