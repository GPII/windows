    ...
    members: {
        ...
        state: {
            ...
            input: {
                ...
                distance: 0,
                lastInputTime: 0,
                // Marks the end of wheel movement
                wheelMovement: 0
            }
        },
 ...
 */
windows.metrics.logAppActivate = function (that, windowInfo) {
    var data = {
        exe: windows.metrics.genericisePath(windowInfo.exe),
 ...
 * @return {Object} Either a RAWINPUTMOUSE or a RAWINPUTKEYBOARD depending on the
 *  contents of the lParam handle.
 */
windows.metrics.getRawInputData = function (lParam) {
    ...
    var res = windows.user32.GetRawInputData(
        lParam,
        windows.flagConstants.RID_INPUT,
        ref.NULL,
        dataSz,
    ...
    if (res === 0) {
        ...
        if (rawInput.header.dwType === windows.flagConstants.RIM_TYPEMOUSE) {
        ...
        promise.reject({ code: errCode, message: "Failed to get GetRawInputData."});
    }
...
windows.metrics.windowMessage = function (that, hwnd, msg, wParam, lParam) {
    ...
    case windows.API_constants.WM_INPUT:
        ...
        pRawInput.then(function (rawInput) {
            if (rawInput.header.dwType === windows.flagConstants.RIM_TYPEKEYBOARD) {
            ...
            } else if (rawInput.header.dwType === windows.flagConstants.RIM_TYPEMOUSE) {
                var relevantMouseEvent =
                    rawInput.mouse.usButtonFlags === windows.API_constants.RI_MOUSE_WHEEL ||
                    rawInput.mouse.usButtonFlags === windows.API_constants.RI_MOUSE_LEFT_BUTTON_UP ||
                    rawInput.mouse.usButtonFlags === windows.API_constants.RI_MOUSE_RIGHT_BUTTON_UP ||
                    // Mouse just moved
                ...
                if (relevantMouseEvent && rawInput.header.hDevice !== 0) {
                    ...
                    var button = 0;
                    if (rawInput.mouse.usButtonFlags === windows.API_constants.RI_MOUSE_WHEEL) {
                        // Unsigned to signed
                        if (wheelDistance >= 0x8000) {
                            wheelDistance -= 0x10000;
                        }
                        if (Math.sign(wheelDistance) === 1 || Math.sign(wheelDistance) === 0) {
                            wheelDirection = 1;
                        } else {
                            wheelDirection = -1;
                        }
                        button = "W";
                    } else if (rawInput.mouse.usButtonFlags === windows.API_constants.RI_MOUSE_LEFT_BUTTON_UP) {
                        button = 1;
                    } else if (rawInput.mouse.usButtonFlags === windows.API_constants.RI_MOUSE_RIGHT_BUTTON_UP) {
                        button = 2;
                    }
 ...
 * @param {Number} hwnd The window handle.
 */
windows.metrics.windowActivated = function (that, hwnd) {
    var state = that.state.application;
    ...
    if (hwnd && hwnd !== state.activeWindow) {
        var windowInfo = state.knownWindows[hwnd];
        if (!windowInfo) {
        ...
        if (windowInfo) {
            ...
            windows.metrics.logAppActivate(that, windowInfo);
        }
    }
...
windows.metrics.getWindowInfo = function (hwnd) {
    ...
    var appClass = "Windows.UI.Core.CoreWindow";
    if (windowInfo.className !== appClass && windowInfo.exe && windowInfo.exe.toLowerCase().endsWith("applicationframehost.exe")) {
        ...
        var child = windows.enumerateWindows(hwnd, function (hwndChild) {
            if (windows.user32.GetClassNameW(hwndChild, classBuffer, classBuffer.length)) {
                ...
                return cls === appClass ? hwndChild : undefined;
            }
        });
        if (child) {
...
windows.metrics.startInputMetrics = function (that) {
    ...
    if (disable) {
        ...
        keyboard.dwFlags =
            windows.flagConstants.RIDEV_NOLEGACY |
            windows.flagConstants.RIDEV_INPUTSINK;
...
windows.metrics.stopInputMetrics = function (that) {
    ...
    // remove the hooks
    if (that.keyboardHookHandle) {
        windows.user32.UnhookWindowsHookEx(that.keyboardHookHandle);
    }
    if (that.mouseHookHandle) {
        windows.user32.UnhookWindowsHookEx(that.mouseHookHandle);
    }
    ...
    if (code >= 0) {
        ...
        case windows.API_constants.WM_KEYUP:
            ...
            if ((eventData.flags & ignoreFlags) === 0) {
                ...
                var keyValue = windows.user32.MapVirtualKeyW(eventData.vkCode, windows.API_constants.MAPVK_VK_TO_CHAR);
                if (specialKey || keyValue) {
                    windows.metrics.recordKeyTiming(that, eventData.time, specialKey, String.fromCharCode(keyValue));
                }
            }
            break;
