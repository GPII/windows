...
windows.bufferToArray = function (buffer, type) {
    if (type === "TCHAR") {
        ...
        return sBuf.substring(0, nullPos);
    } else {
        var array = [];
        var size = ref.coerceType(windows.types[type]).size;
 ...
 *
 * @param {Object} color The color in RGB format, as and object of type: { r: 0x00, g: 0x00, b: 0x00 }
 * @returns {Number} Non-zero if operation succeeded, zero otherwhise.
 */
windows.setSolidColor = function (rgbColor) {
    var color = rgbColor.r | rgbColor.g << 8 | rgbColor.b << 16;
    var bufElems = ref.alloc(t.INT, windows.API_constants.COLOR_DESKTOP);
    var bufColors = ref.alloc(t.UINT, color);

    return windows.user32.SetSysColors(1, bufElems, bufColors);
};

/**
 * Returns the current desktop solid color as a RGB color object of format: { r: 0x00, g: 0x00, b: 0x00 }
 *
 * @returns {Object} The current desktop solid color.
 */
windows.getSolidColor = function () {
    var decimal = windows.user32.GetSysColor(windows.API_constants.COLOR_DESKTOP);

    return { r: (decimal & 0x0000ff), g: (decimal & 0x00ff00) >> 8, b: (decimal & 0xff0000) >> 16 };
}
