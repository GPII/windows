...
jqUnit.asyncTest("serviceChallenge tests", function () {
    ...
    badHandles.forEach(function (handle) {
        ...
        gpii.windows.serviceChallenge(handle);
    });
    ...
    gpii.windows.serviceChallenge(eventHandle);
});
...
jqUnit.asyncTest("connectToService failure tests", function () {
    ...
    var serviceHandler = gpii.windows.serviceHandler({
        listeners: {
            // Don't auto-connect.
    ...
    promise.then(function () {
        ...
        jqUnit.assertTrue("connectToService should reject with an error", e instanceof Error);
        jqUnit.assertEquals("connectToService should reject with ENOENT", "ENOENT", e.code);
        runTest(0);
    });
...
jqUnit.asyncTest("connectToService tests", function () {
    ...
    var runTest = function (testIndex) {
        ...
        var pipeId = "test-connectToService-" + testIndex;
        var pipeName = serviceHandler.pipePrefix + pipeId;
        ...
        pipeServer.on("connection", function (pipe) {
            ...
            pipe.on("data", function (data) {
                fluid.log("pipeServer.data", data);
            });
