...
gpii.tests.windows.processHandling.startTestProcess = function (output) {
    ...
    fluid.log("Executing ", exePath, args.join(" "));
    var child = child_process.spawn(exePath, args, {encoding:"utf8"});
...
jqUnit.asyncTest("Testing close process after command", function () {
    jqUnit.expect(3);

    gpii.windows.waitForProcessStart("notepad.exe")
        .then(function () {
            jqUnit.assert("Process started");

            gpii.windows.waitForProcessStart(waitExe)
                .then(function () {
                    jqUnit.assert("WaitExe process started");
                    gpii.windows.closeAfterExitCommand("taskkill", ["/IM", "notepad.exe"], "notepad.exe", waitExe)
                        .then(function () {
                            jqUnit.assert("Process terminated");
                            jqUnit.start();
                        }, function (err) {
                            jqUnit.fail("Error executing command and closing process. Error: " + err);
                        });
                }, function (err) {
                    jqUnit.fail("Should not be an error in process openning. Error: " + err);
                });
