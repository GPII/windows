...
gpii.tests.doProcessKillTest = function(exeName, exePath, args) {
    jqUnit.expect(3);

    var pid = gpii.windows.findProcessByName(exeName);
    jqUnit.assertEquals("The process should not already be running.", null, pid);
    ...
    shelljs.exec(command, {
        ...
        // Why 4294967295? .NET framework Process.Kill uses -1 as the exit code per
        // http://referencesource.microsoft.com/#System/services/monitoring/system/diagnosticts/Process.cs,2376 
        // Process.Kill wraps TerminateProcess() from Windows API in kernel32.dll. TerminateProcess
        // treats the exit code as a UINT (32-bit) per https://msdn.microsoft.com/en-us/library/windows/desktop/ms686714(v=vs.85).aspx
        // this causes -1 to wrap around to max value of an unsigned 32-bit integer which is 4294967295.  
        jqUnit.assertEquals("Process should have terminated with code SIGKILL", 4294967295, code);
...
jqUnit.asyncTest("Testing Killing Processes", function () {
    gpii.tests.doProcessKillTest(waitExe, waitExePath, "killProcessByNameTest /T 30");
});

jqUnit.asyncTest("Testing Killing Processes (64-bit)", function() {
    var exeName = "mspaint.exe";
    var exePath = path.join(process.env.SystemRoot, "System32", exeName);
    gpii.tests.doProcessKillTest(exeName, exePath);
});

jqUnit.asyncTest("Testing Killing Processes (32-bit)", function() {
    var exeName = "notepad.exe";
    var exePath = path.join(process.env.SystemRoot, "SysWOW64", exeName);
    gpii.tests.doProcessKillTest(exeName, exePath);
});
