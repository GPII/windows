...
jqUnit.test("Testing wmiSettingsHandler set new brightness using the setImpl payload", function () {
    ...
    if (supportedBrightness) {
        try {
            ...
            var newBrightnessVal = result.Brightness.newValue;
            ...
            var oldBrightnessPayload = ["root\\WMI" ,"WmiMonitorBrightnessMethods", "WmiSetBrightness", [0xFFFFFFFF, oldBrightnessVal], ["uint", 0]];
...
jqUnit.test("Testing wmiSettingsHandler set handles unsupported feature.", function () {
    ...
    if (notSupported) {
        var result = windows.wmi.setImpl(payload);
        var nullOld = result.Brightness.oldValue === null;
        var nullNew = result.Brightness.newValue === null;
...
jqUnit.test("Testing wmiSettingsHandler set handles null/non null oldValue", function () {
    ...
    var mockWindows = windows;
    mockWindows.wmi.getImpl = function (payload) {
    ...
    var result = mockWindows.wmi.setImpl(payload);
    var oldValue = result.Brightness.oldValue === 20;
    var correct = oldValue;
    jqUnit.assertTrue("The handler should attempt to set value when setting is supported.", correct);
});
