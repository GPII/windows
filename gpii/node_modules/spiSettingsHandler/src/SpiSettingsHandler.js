...
gpii.windows.spi.setImpl = function (payload) {
    ...
    if (payload.settings.HighContrastOn && !payload.settings.HighContrastOn.value) {
        var currentState = gpii.windows.spi.getImpl(fluid.copy(payload));
        if (!currentState.HighContrastOn.value) {
            togo.resolve({
                HighContrastOn: { oldValue: payload.settings.HighContrastOn, newValue: payload.settings.HighContrastOn }
            });
        }
    }
