...
gpii.windows.spi.setImpl = function (payload) {
    ...
    if (payload.settings.HighContrastOn) {
        ...
        if (doNothing) {
            ...
            var cachedFilesPathEnv = "%APPDATA%\\Microsoft\\Windows\\Themes\\CachedFiles";
            var cachedFilesPath = cachedFilesPathEnv.replace(/%([^%]+)%/g, function (_,n) { return process.env[n] });
            var optionsCached = {
                "recursive": true,
                "silent": true,
                "local": true
            ...
            var tWallpaperEnv = "%APPDATA%\\Microsoft\\Windows\\Themes\\TranscodedWallpaper";
            var tWallpaper = tWallpaperEnv.replace(/%([^%]+)%/g, function (_,n) { return process.env[n] });
            var optionsCached = {
                "silent": true,
                "local": true
            };

            gpii.windows.rm(cachedFilesPath, optionsCached);
            gpii.windows.rm(tWallpaper, optionsCached);
        }
    }
