...
gpii.windows.spi.waitForSpi = function (action, exeFile) {
    ...
    var options = {
        ...
        env: {
            GPII_CHILD_PROCESS: 1,
            GPII_SETHC_EXE: exeFile || "sethc.exe"
        }
    };

    // If debugging, remove the debugger listeners
    if (typeof v8debug === "object") {
        options = { execArgv: [] };
        for (var i = 0, len = process.execArgv.length; i < len; i++) {
            if (!process.execArgv[i].match(/--debug(-brk)?(=.*)?/)) {
                options.execArgv.push(process.execArgv[i]);
            }
        }
    }

    var child = cp.fork(__dirname + "/SpiHighContrastCompletion", options);
...
gpii.windows.spi.applySettings = function (payload) {
    ...
    callSuccessful = gpii.windows.spi.systemParametersInfo(pvParamType, action, uiParam, pvParam);

    var togo = pvParam;

    if (callSuccessful) {
        togo = gpii.windows.spi.waitForSpi(action);
    } else {
        var errorCode = gpii.windows.kernel32.GetLastError();
        fluid.fail("SpiSettingsHandler.js: spi.applySettings() failed with error code " + errorCode + ".");
    }

    return fluid.toPromise(togo);
};
